...
export function collectSceneOptionProperties(buffer: TextBuffer, member: SceneSourceObject): SceneOptionProperty[] {
		...
		if (field.value.kind === LuaSyntaxKind.TableConstructorExpression && field.value.fields.length > 0) {
			...
			for (const child of field.value.fields) {
				...
				const key = child.kind === LuaTableFieldKind.IdentifierKey ? child.name
					: child.kind === LuaTableFieldKind.ExpressionKey ? `[${readLuaSourceRange(buffer, child.key.range)}]` : `[${++arrayIndex}]`;
				append(child, label === '' ? key : `${label}${key[0] === '[' ? '' : '.'}${key}`);
			}
		} else {
			const sourceText = readLuaSourceRange(buffer, field.value.range);
			properties.push({ field, label: label === '' ? 'options' : label, sourceText, editable: !sourceText.includes('\n'),
				preview: readLuaExpressionPreview(buffer, field.value), text: '', contentBounds: create_rect_bounds(), bounds: create_rect_bounds() });
		}
	};
