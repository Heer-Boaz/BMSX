...
export const STUDIO_RUNTIME_TOOLS = [
	...
	{ name: 'studio_read_frame_scopes', description: 'Read named locals/upvalue/static scopes for a frame handle from this inspection\'s stack. Use studio_read_runtime_values on their references. Locals use installed lexical/inline scopes and register liveness; unavailable locations are explicitly unavailable, not nil or guessed constants. Shadowed names retain distinct declaration ranges. Upvalues belong to the physical closure; an inline frame has no separate closure. Static storage entries carry exact address words, not memory contents; struct types are explicit non-values. No Lua execution or frame-context evaluation.',
		inputSchema: { type: 'object', properties: { frame: { type: 'string' } }, required: FRAME_FIELDS, additionalProperties: false } },
	{ name: 'studio_read_runtime_values', description: 'Read a page from a scope/table reference in this prompt\'s current suspended inspection. Zero-based start, positive count. Values and table keys retain guest kinds; displays are not lookup keys. Local/upvalue entries carry isConst for the installed binding, independently of location availability; this does not freeze table contents. Tables expose stored entries only, without executing metamethods. Repeated/cyclic tables share a reference. Number displays preserve guest formatting. A page past the end is empty, not missing values. Expired references require a new inspection.',
