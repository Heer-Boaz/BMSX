...
export class WorkspaceSourceTools {
	...
	private readonly receipts = new Map<string, CapturedWorkspaceSource>();
		...
		switch (request.name) {
			...
			case 'studio_propose_edits': {
				...
				for (const file of request.files) {
					const captured = this.receipts.get(file.receipt);
					if (!captured) throw new SourceToolInputError('Every edited file needs a receipt read in this source context');
