...
export type TestInspectionRequest =
	...
	| { name: 'studio_read_test_stack'; failure: string; start: number; count: number }
...
const RESULT_FIELDS = ['result'], FAILURE_FIELDS = ['failure', 'start', 'count'], FRAME_FIELDS = ['frame'], VALUE_FIELDS = ['reference', 'start', 'count'];
...
export const STUDIO_TEST_INSPECTION_TOOLS = [
	...
	{ name: 'studio_read_test_stack', description: 'Read a page of one retained test failure\'s own thread stack, top first. Never substitutes the current/authoring CPU. Physical domain, address and PC remain machine values; resource domain identifies the original authored socket. Source locations are one-based and refer to the compiled test images, not current working copies. Inline and recursive frames have distinct handles. Zero-based start, positive count.',
		inputSchema: { type: 'object', properties: { failure: { type: 'string' }, ...PAGE }, required: FAILURE_FIELDS, additionalProperties: false } },
	{ name: 'studio_read_test_frame_scopes', description: 'Read locals/upvalue scope handles for a frame from the retained test stack. Actual frame register windows and closure captures, with compiler liveness: optimized/unavailable locations are explicit, not nil. Inline frames have no separate closure. Read values with studio_read_test_values; no Lua evaluation.', inputSchema: FRAME_SCHEMA },
	{ name: 'studio_read_test_frame_source', description: 'Read the exact compiled source of a retained test frame, including accepted unsaved test/dependency text or BIOS/companion ROM source. Never reads a newer editor model in its place. Instruction-only frames explicitly report unmapped source. No edit or save authority.', inputSchema: FRAME_SCHEMA },
	{ name: 'studio_read_test_values', description: 'Page a retained test scope or stored table entries. Guest kinds and typed keys are preserved; cyclic tables share a reference. No metamethods or Lua code run. Values belong to the case-end retained heap, not the historical failure instant. Zero-based start, positive count; a page past the end is empty.',
...
export function decodeTestInspectionRequest(name: string, input: unknown): TestInspectionRequest {
	switch (name) {
		...
		case 'studio_read_test_frame_scopes': case 'studio_read_test_frame_source': {
			...
			if (typeof value.frame !== 'string') throw new StudioToolInputError('frame must be a handle from the retained test stack');
			return { name, frame: value.frame };
		}
		case 'studio_read_test_stack': case 'studio_read_test_values': {
			const stack = name === 'studio_read_test_stack', value = toolArguments(input, stack ? FAILURE_FIELDS : VALUE_FIELDS);
			const reference = stack ? value.failure : value.reference;
			...
			return name === 'studio_read_test_stack'
				? { name, failure: reference, start: value.start as number, count: value.count as number }
