...
export const STUDIO_TEST_INSPECTION_TOOLS = [
	...
	{ name: 'studio_read_test_frame_scopes', description: 'Read locals/upvalue/static scope handles for a frame from the inspected test stack. Actual frame register windows and closure captures, with compiler liveness: optimized/unavailable locations are explicit, not nil. Inline frames have no separate closure. Static storage entries carry exact address words, not memory contents; struct types are explicit non-values. Read values with studio_read_test_values; no Lua evaluation.', inputSchema: FRAME_SCHEMA },
