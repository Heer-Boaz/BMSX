...
local event_matcher_syntax<const> = require('cartlib/event_matcher_syntax')
...
function event_matcher.compile(matcher)
	...
	local syntax_tree<const>, capture_plan<const> = event_matcher_syntax.build(matcher)
	local environment<const> = {}
	if capture_plan.uses_payload_fields then
		environment.operands = capture_plan.operands
		environment.value_type = type
	end
	if capture_plan.uses_any_matches then
		environment.any_matches = any_matches
	end
	if capture_plan.uses_list_contains then
