-- TODO: ONGEBRUIKT, TERWIJL IK HAD VERWACHT DAT EEN CART ALS `pietious` DEZE WEL ZOU GEBRUIKEN!

-- reusable pool of world objects driven by definition arrays (data)
--
-- DESIGN PRINCIPLES
--
-- 1. THE POOL MANAGES OBJECT LIFETIME SO CART CODE DOESN'T HAVE TO.
--    Given an array of "definition" tables (e.g. enemy spawn points from
--    a data-struct such as a YAML file), the pool creates, activates, syncs,
--    and deactivates objects in one call each frame:
--
...
-- object_pool:begin_cycle(): clears the active-ids set for this frame.
--   Must be followed by one or more :use() calls and then :end_cycle().
function object_pool:begin_cycle()
	clear_map(self.active_ids)
end

-- object_pool:use(definition, context)
...
function object_pool:use(definition, context)
...
function object_pool:mark_active(id)
	self.active_ids[id] = true
end

-- object_pool:end_cycle(): deactivates all objects that were NOT used
--   this cycle (i.e. absent from the definitions list). Cleans up stale ids
--   for objects that no longer exist in the world.
function object_pool:end_cycle()
...
function object_pool:deactivate_id(id)
...
-- object_pool:sync_array(definitions, include_definition?, context)
...
--   calls end_cycle. This is the main entry point for data-driven pools.
function object_pool:sync_array(definitions, include_definition, context)
	self:begin_cycle()
