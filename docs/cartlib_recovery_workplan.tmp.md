...
- Alle Lua-modules en identifiers gebruiken `snake_case`; de ownernamen hieronder
  zijn de werkelijke Lua-namen en geen hoofdletterige engine-types.
...
- De oude TypeScript-engine is alleen referentie voor ownership. Oude
...
`world` is de enige owner van objecten, ids, `space`-partities, lifecycle en object-, tag-,
...
### `world_renderer` en `presentation`

`world_renderer` leest de retained visualview en bouwt commands in één retained
...
- Vervang het render-ECS-system door `world_renderer` en `presentation`.
