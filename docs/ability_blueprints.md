...
- **Ability Steps** describe the work performed by an ability. Supported step kinds:
  - `call` &mdash; invoke a registered gameplay action with optional parameters.
...
- **GameplayActionRegistry** maps string identifiers to TypeScript functions. `call` steps run these gameplay actions with a rich context (owner, tags, vars, command buffer, nested ability requests).
...
1. Register domain-specific gameplay actions on a `GameplayActionRegistry`:
   ```ts
   const actions = new GameplayActionRegistry();
...
2. Describe abilities with plain objects, using the helper value builders exported from `bmsx/gas/gameplay_ability` (`literal`, `fromIntent`, `fromVar`, etc.).
   ...
   const walkAbility: GameplayAbilityDefinition = {
     ...
     activation: [
      { type: 'call', gameplayAction: 'fighter.configureWalk', params: { direction: fromIntent('payload.direction', { optional: true }) } },
...
- The gameplay action registry encapsulates operations like walking configuration, jump dispatch, attack bookkeeping, and flying-kick gating without scattering logic across abilities.
