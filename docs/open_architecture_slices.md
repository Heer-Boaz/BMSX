...
| 27 | In progress | Lua VM heap representation | Table-hash storage is nu owner-level columnar storage; resterende small-object druk zit nog bij closures/upvalues/native handles en moet met metingen per owner verder omlaag. | Steady-state heapmeting toont structureel minder kleine allocaties zonder extra guards, fallbacks of hot-path GC-churn. |
| 28 | Open | Program image runtime residency | Resterende program-runtime bytes alleen reduceren na bewijs van dubbele rodata/constpool/code ownership in de program loader/runtime. | Gemeten program-image live bytes dalen zonder debug-symbol afhankelijkheid, legacy reader, of TS/C++ representatie-drift. |
