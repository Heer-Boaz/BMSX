...
| `STUDIO-BEHAVIOR-UX-B03` | Same-document BT reparent-gesture gebouwd: bronfeiten, transfer/binding, review en Undo. Zie `behavior_tree_reparenting.md`. | Vóór/na/in, lege parent/enige member, shared uses en initialisatie-review bewezen met echte pointer/Source/Undo op drie renderers. De afzonderlijke reparent→Save→live-BT-Hot-Resume→Undo-proef bewijst gewijzigde taakvolgorde en behouden component/blackboard via ICU-input. Cycles/lexical bindings en compiled task/evaluation-order getest; geen cross-fileoperatie of sluiting van B04. |
| `STUDIO-BEHAVIOR-UX-B04` | Resource-eigen bronherkomst, write-targets en snapshot-/dependencygeldigheid binnen de bestaande declaratieve cartlib-norm. De scope is op 2026-09-13 aangescherpt in `behavior_definition_inspection_design.md` (D1); onbeperkte factory-/wrapperreconstructie is geen gate voor alle UX. | Imports, gedeelde declaraties, callbackbron en unsaved dependencies behouden correcte Source/Back en Undo. Geen runtimepublicatie claimen op basis van modulepad/const-spelling of één kandidaat. De gedeelde edit/history-owner is gebouwd en aangesloten op cross-file Rename (`editor_workspace_history.md`); graph-input/Save/focus en cross-resource binding-admission blijven open. Geen tweede semantic engine of gekookt behaviorresource. |
...
| `STUDIO-BEHAVIOR-UX-B06` | Presentatie/inspectie en authored property-edit gebouwd: twaalf rollen, requirements eenmaal, callbackbron en echte expression-cellen. Zie `actioneffect_properties_design.md` en `actioneffect_authoring.md`. | Draft/cancel/invalid Enter/Save en één bron-Undo; multiline en foreign write resources gaan naar hun eigen broneditor. Echte Studio-proeven op drie renderers; geen tweede working copy, runtime-evaluatie of extra cartlib-metadata. Nieuwe properties/factory-instanties en het bredere B04-callcontextcontract worden niet hiermee afgevinkt. |
