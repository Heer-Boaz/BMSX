...
| `STUDIO-FSM-SOURCE-GRAPH-01` | Na BT: maak containment, entry en aantoonbare mogelijke transitions typed sourcefeiten met originscope, target en bewijs. Dynamische relaties krijgen geen verzonnen endpoints; callbackreturns zijn alleen transitions in bewezen consumer-slots. | Fixtures volgen `fsm.lua`/`fsm_component.lua` voor ondersteunde paths, scopes, guards, callbacks en meerdere machines. Parenthandlers worden niet willekeurig childedges. Cross-file bewijs vereist semantic-generation-invalidering; geen name matching, host-Lua-uitvoering of FSM-regels in de generieke binder. |
