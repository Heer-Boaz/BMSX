...
| `CPU-IMAGE-DATAPATH-01` | Vervang `Blua32ImageLayout` en gekopieerde hostobjectgrafen als uitvoeringsautoriteit door reads van functie-, constante-, naam- en instructierecords via de fysieke execution-address-space. Alleen een decoded instruction cache mag blijven als afgeleide cache van fysieke woorden, nooit als tweede executable reality. | TS en C++ voeren dezelfde fysieke recorddatapath uit; vreemde maar representeerbare bits leveren deterministisch resultaat en geen decoded hostobjectgraf bepaalt CPU-gedrag. |
