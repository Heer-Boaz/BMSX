...
| `STUDIO-FSM-GRAPH-VIEW-01` | Consumeer na compound-layout, layoutlifetime en bronselectie de bewezen FSM-projectie in de gedeelde canvas. Verbind de concrete broninput met die lifetime; één callbackrij kan meerdere edges leveren. Source-edit/close mag geen oud layoutresultaat publiceren. | Zelfstandige fixtures en live 384×288-captures bewijzen leesbare scopes en afzonderlijk selecteerbare edges met hun eigen bronbewijs. Geen BT-tree-/aigen-DAG-normalisatie, BT-`edgesBySource` als many-edge-identiteit of herroutering naar een ingeklapte ancestor. De ELK-keuze en nog ontbrekende inputgrenzen staan in `behavior_graph_design.md`; de FSM blijft tot deze slice outline. |
