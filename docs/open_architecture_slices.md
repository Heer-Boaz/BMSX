...
| `NEMESIS-ROOT-SCENE-01` | Verplaats uitsluitend de vier statische rootspawns van `carts/nemesis_s/cart.lua` naar één cart-owned registered Lua-scene. Gebruik de bestaande definition-/instance-id exports, Spaces, pos-waarden en authored order; registratie gebeurt in dezelfde expliciete `<init>` als FSM/BT/prefab-registration. | Nemesis boot, cinematic flow, game-over/new-game en pause-scenario's blijven groen; release-O3-meting blijft binnen de vastgelegde koude overhead en vertoont geen verschil in de vijfseconden-framecategorieën na initialization. Stage/director/enemy/effectspawns blijven cart-owned gameplay. |
