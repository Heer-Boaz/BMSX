...
| `IDE-LUA-FULL-FIDELITY-EDIT-DESIGN-01` | Ontwerp insert/remove/reorder van Lua-table fields/entries pas vanuit een syntaxrepresentatie die ook separators en comment-/whitespace-trivia bezit. De huidige AST-fieldrange bevat die broninformatie niet; zij met feature-local textscans reconstrueren is geen source-preserving architectuur. | Het ontwerp vergelijkt de live lexer/parser met Roslyn-achtige full-fidelity tokens, bepaalt trivia-ownership en incremental-/memorykosten, en noemt alle compiler, semantic, formatter en IDE-consumenten vóór een parserwijziging. Geen incomplete table-rewriter als voorportaal voor de scene-editor. |
