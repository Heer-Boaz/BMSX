...
| `REWIND-UI-01` | Game-first bottom transport, host tiny font, retained render commands; LB/RB seek, START/A accepts, B cancels, pointer selects time. One overlay transition owner handles departure, input reset and destination activation; keyboard opening has no rewind cleanup. | TS/C++ implementation replaces the rejected action-list submenu. Real-host tests cover between-checkpoint replay, repeat/clamp, return/branch, keyboard transitions and held opening buttons. TS additionally accepts a target while GPU readback is pending; browser WebGPU compares restored VRAM and captures the actual transport. Validation is recorded in the rewind document. Snapshot capacity and capture cadence are unchanged. |
