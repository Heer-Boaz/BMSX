...
| `STUDIO-ACTIONEFFECT-PERIODIC-TRACE-01` | Voeg daarna uitsluitend voltooide periodieke ActionEffect-uitvoering toe aan dezelfde geselecteerde componentstream. `tick_periodic()` commit eerst de volgende deadline, voert `execute_effect()` uit en publiceert pas na return `periodic_execute`, effect-id en de rauwe scheduled gameplay-deadline die due was. Geneste trigger/activityfacts en handler-deactivation blijven daardoor vóór de completion staan; lane-removal blijft de bestaande daaropvolgende ownerstap. De recorder behoudt dezelfde vaste recordvorm, directe kindstring en producer sequence. | Een compiler/runtime-test bewijst deadline-before-handler, completion-after-nested-facts, geen completion bij een fault, fixed-buffer-overflow, nul post-warm-up guestallocaties en gemeten enabled overhead. De echte Nemesis held-fireflow toont in één result sequence `activate 1`, `trigger accepted`, `periodic_execute <scheduled-ms>`, `deactivate 0`, terwijl de bestaande projectiel- en cadenceasserties gameplay bewijzen. Gewone release/debug/Hot-Resume-bytecode bevat geen ActionEffect-tracekanaal, `periodic_execute`-constant of sinkdispatch. Geen cooldowncommit-, grant/revoke/rebind-, event-, projectiel- of schedulerinferentie, tweede recorder, host-tijdmerge, algemene periodic callbackfacade, BT-identiteit of machine/C++-wijziging. |
