...
| `STUDIO-BEHAVIOR-RESOURCE-DESIGN-01` | Kies de authored representatie vóór een editable graph. Combineer VS Code's text-backed custom-editorlifecycle met afzonderlijke typed resources: BT als ordered tree, FSM als state/transitionresource en ActionEffects als typed collectie. JSONC is canoniek; stable UUID's identificeren authored elementen, terwijl namen, layout en runtime-dense slots dat niet doen. Lua-implementaties blijven expliciete cart-owned bindings die iedere behaviorlibrary eenmaal tijdens admission naar directe closures verlaagt. De packer kookt naar gewone cartdata; machine, TOC, sockets, C++ en cartlib kennen geen Studio. | **Geaccepteerd:** `studio_functional_design.md` specificeert document-, element-, layout-, binding-, cook- en runtimegrenzen met gepinde VS Code, Microsoft JSONC, BehaviorTree.CPP, Godot en Unity-referenties. D en E zijn geen alternatieven meer: het structured resource is een tekstwerkdocument. Geen YAML-roundtrip, generiek graph-DTO, globale callbackregistry, nieuw machine-assettype of automatische Lua-migratie. De eerste productroute is BT en vereist eerst de twee volgende prerequisites. |
| `IDE-TEXT-DOCUMENT-OWNER-01` | Maak het bestaande editor-textmodel werkelijk resource-owned voordat een tweede view kan editen. Een retained documentmodel per `(domain,path)` bezit PieceTree-buffer, versie, dirty/save/applied-generaties en undo/redo. Code- en toekomstige visual-editorinputs bezitten alleen viewstate en delen datzelfde model. Programmatic structured edits richten zich op een documentmodel, worden als één undo-element toegepast en publiceren dezelfde change event als typen, undo, redo en revert. | Twee gelijktijdige views op één fixture zien typen, programmatic multi-edit, undo, redo, save en revert elk eenmaal en in dezelfde versievolgorde; sluiten van één view verwijdert het gedeelde dirty model niet. Autosave, rename, diagnostics, Hot Resume en navigation lezen dezelfde models zonder active-tabkopie of background-buffer special case. Geen facade rond `editorDocumentState`, parallelle undo stack, mode-switch op optionele DTO-velden of volledige documentreplace voor een property-edit. Bestaande Lua/AEM IDE-tests, browser-Studiobuild, indent en architectuuraudit zijn groen. |
| `BT-AUTHORED-RESOURCE-01` | Voeg na de documentowner het eerste typed authoringresource toe: `*.bt.jsonc`. De toolchain parseert met Microsoft's JSONC-parser, valideert een versioned BT-schema en kookt het naar een gewone `data`-asset zonder editorlayout. De authored ordered tree bewaart stabiele UUID's voor nodes, decorators en Services en verwijst naar blackboard- en cartbinding-id's. Een BT-owned admissionmodule decodeert het asset eenmaal, bindt de door cartcode geleverde Task/Service/decoratormanifests en voert de bestaande `program.compile` uit. | Packerdiagnostics dragen exacte JSONC-ranges; comments en formatting overleven gerichte edits. Een echte cartfixture importeert alleen het gewone `bmsx/assets`-adres, registreert een resource vanuit een expliciet lokaal bindingsmanifest en draait hetzelfde allocatievrije compiled framepad als een Lua-authored tree. Hot Resume van semantiek of bindingcode kookt/installeert media via de bestaande ROM-owner en rerunt de bestaande annotated cart-init; een inkompatibele static-layoutedit wordt eerlijk geweigerd. Geen gegenereerde Lua-module, runtime-JSON-parser, Studio/cartlib-import, machine/TOC/C++-type, global bindingregistry, per-frame stringlookup of poging om arbitrary Lua te herschrijven. |
| `STUDIO-BT-VISUAL-EDITOR-01` | Bouw pas na beide prerequisites een echte host-side BT custom text editor op het canonieke `*.bt.jsonc`-model. De retained projection toont ordered topology, attachments, blackboard en typed properties; alle add/remove/move/connect/propertyacties produceren minimale JSONC-textedits via dezelfde document-undo-owner. Bron- en visual view zijn gelijkwaardige editorinputs op hetzelfde model. | Keyboard, controller en pointer kunnen op 384x288 met het IDE-tiny-font een tree maken en wijzigen; selectie, collapse, viewport en hover blijven viewstate, terwijl iedere semantische of gedeelde-layoutwijziging dirty/save/revert/Hot Resume volgt. Split text/visual fixtures bewijzen bidirectionele synchronisatie, stable-idbehoud bij rename/move en nieuwe UUID's bij copy/paste. Draw parseert, formatteert of alloceert geen topology per frame. Geen React/DOM-workbench, guest viewport, tweede graphdatabase, whole-document stringify, Lua-tablemutatie of observertrace als authored waarheid. |
