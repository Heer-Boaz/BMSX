...
| `IDE-LUA-FULL-FIDELITY-EDIT-DESIGN-01` | Ontwerp insert/remove/reorder van Lua-table fields/entries pas vanuit een syntaxrepresentatie die ook separators en comment-/whitespace-trivia bezit. De huidige AST-fieldrange bevat die broninformatie niet; zij met feature-local textscans reconstrueren is geen source-preserving architectuur. | **Lexicale grens vastgesteld, structurele gate blijft open:** `lua_source_syntax_design.md` inventariseert compiler-, semantic-, formatter-, encoding- en IDE-consumenten en vergelijkt TypeScript, Roslyn en Full Moon. De lexer kan trivia behouden zonder die aan normale analyses op te leggen. Parser-owned punctuated fields, trivia-attachment, malformed-source-representatie en gedeelde source-versionopslag zijn nog geen implementatie; geen add/remove/reorder op basis van een tokenstream alleen. |
