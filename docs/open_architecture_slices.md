...
| `IDE-LUA-NUMERIC-LITERAL-EDIT-01` | Voeg bij de generieke Lua-language-owner uitsluitend vervanging van een bestaande signed numeric literal toe. De bewerking gebruikt de bestaande syntaxrange, behoudt de aanwezige decimale/hexadecimale schrijfconventie waar die representatie dat toelaat en levert één `EditorTextEdit` aan het bestaande textmodel. | Tests met positieve/negatieve decimalen, hexwaarden, multiline tables en comments bewijzen een minimale range, exact behoud van alle overige bytes en één model-undo-element voor een batch. Een dynamische expressie is geen numeric-literal-edittarget. Geen scene-/BT-/FSM-type, whole-document serializer of parallelle undo-owner. |
