...
FSM-/BT-/ActionEffect-patronen: opt-in admission, alleen verlagen voor gemeten
herhaald werk, alleen noodzakelijke runtime-state en Hot Resume bij de echte
consumer. De eerstvolgende slice bewijst het voordeel op huidige carts voordat
een nieuwe cartlib-owner of recordshape wordt gekozen.
...
| `STUDIO-SCENE-RUNTIME-DESIGN-02` | Leid scene-admission af van de bestaande FSM-, BT- en ActionEffect-owners: registration/rebind alleen voor levende consumenten, verlaging alleen wanneer zij herhaald werk verwijdert en directe guestwaarden zonder DTO-schema. Toets dat aan Godot- en Defold-compositie zonder hun native/desktop-opslag als Lua-heapmodel te kopiëren. | **Ontwerprichting geaccepteerd, runtime nog open:** een cart zonder scene-import blijft byte- en runtimekostenneutraal; `World` krijgt geen scene-branch of tweede commandbuffer. Directe Lua is baseline voor kleine assembly; opt-in definitions, een verlaagd programma en immutable placementdata worden alleen op echte workloads vergeleken. |
| `STUDIO-SCENE-WORKLOAD-01` | Meet buiten productiecode de huidige directe rootassemblies van `2024`, `nemesis_s`, `pietious` en `2025`, plus een kleine en de grootste Pietious-room en Nemesis' stageplacements. Vergelijk uitsluitend representaties die de bestaande cartlibpatronen volgen: directe Lua, ActionEffect-achtige directe definition, BT/FSM-achtige verlaging en immutable placementdata. | Per workload staan concrete authoringwinst, ROM-bytes, statische instructies/cycles, init tables/closures, load/reload/roomwissel en normale update/renderkosten vast. `2024` en een cart zonder scene-import zijn negatieve controles. De uitkomst mag verschillende representaties voor rootassembly en placementcollections kiezen; geen productie-`World`-, `prefab`-, machine- of Studio-runtime-edit in deze slice. |
