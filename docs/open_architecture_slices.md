...
| `REWIND-HISTORY-01` | Runtime-owned bounded checkpoints and external ICU events, including unarmed supervisor edges. Machine-time seek with backend fence service, no live input or vibration during replay; hosts drain intermediate delivery. Seeking retains future; live takeover branches. | Implemented shared core, disabled by default. Real BIOS/Nemesis conformance proves paced recording, both ring wraps, sparse quantum seeks, live takeover and full TS/C++ post-branch equality. Mirrored tests cover pressure, cancellation and submitted backend waits. Capture/replay costs and retained memory are measured in the rewind document; CPU graph storage is now retained under `REWIND-CPU-STORAGE-01`; remaining device/capture allocations still require a host budget before user-facing enablement. |
