...
Alle onderstaande rijen die een scene-runtime, scene-sourcevorm of
scene-runtimebinding veronderstellen zijn geparkeerd tot
`STUDIO-SCENE-RUNTIME-DESIGN-02` een vervangend contract oplevert. Hun oude
detailtekst is geen implementatieopdracht.
...
| `STUDIO-SCENE-AUTHORING-DESIGN-01` | Ontwerp vóór runtime of UI de scene-representatie en editworldgrens tegen volwassen productievoorbeelden en de live BMSX-owners. | **Runtimegedeelte verworpen:** canonical Lua, gedeeld textmodel, host-side visual view en geen machine-/ROM-/TOC-lek blijven geldig. De voorgestelde `SceneInstance`/structural-batch/propertyrepresentatie introduceerde onbewezen guestheap- en basiskosten en is volledig verwijderd. [`studio_scene_authoring_design.md`](studio_scene_authoring_design.md) bewaart de meting en de verworpen hypothese. |
| `STUDIO-SCENE-RUNTIME-DESIGN-02` | Begin opnieuw vanuit de 33.8688 MHz guest. Meet bestaande productiecart-load/spawn/clear/Hot-Resume, Defolds vooraf begrensde collection/instanceopslag en Flecs' native staging zonder hun C-layout als Lua-heaprecords te kopiëren. Bepaal daarna pas of scenes een optionele cartlibmodule, bestaande cartcodeconventie of gerechtvaardigde cooked runtime-representatie nodig hebben. Ontwerp geen propertyschema voordat een echte prefabowner en editorbewerking het vereisen. | Voor iedere voorgestelde runtimewaarde staan owner, levensduur, guestwoorden/heapobjecten, ROM-kosten en dynamische instructies vast. Een release-O3-cart zonder scenegebruik is byte-, init-, heap-, spawn-, update- en renderkostenneutraal. Representatieve Nemesis- en grote-sceneproeven begrenzen load, levelwissel en Hot Resume. Geen wijziging aan `World`, `prefab` of machine vóór dit contract geaccepteerd is; geen algemene Lua-commandbuffer, typelabeltabel, DTO-validator of desktop-enginekopie. |
