...
Afgeronde follow-up (24.1b): `machine_manifest.vdp_mode` en de daarvan afgeleide
`machine_manifest.render_size` zijn uit guest Lua weg. De ROM/header kiest
hoogstens de VDP-class; de cart owner zet de VDP-mode zelf via het VDP-mode
MMIO-register. De live zichtbare dimensies komen uit het VDP-owned
`sys_vdp_screen_wh` registerwoord.
