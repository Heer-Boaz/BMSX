...
begint met A + C: eerst een read-only source-first Behavior Lens, daarna een
deterministisch Scenario Lab. Dat is infrastructuur en geen observer-only
eindproduct. Studio moet daarna echte visual authoring bieden voor onder meer
BT-, FSM- en ActionEffect-data. `STUDIO-BEHAVIOR-RESOURCE-DESIGN-01` kiest
daarvoor typed JSONC-resources met één gedeeld textdocument als canonical owner;
een traceprojectie wordt nooit stilzwijgend het editable graphmodel. Inspectie is aanvankelijk
stop-and-inspect, arbitrary Lua blijft source-first, exacte per-node
runtimecorrespondence valt buiten de eerste lens en libretro blijft
speler/core. Geen van deze keuzes maakt Studio tot machine-, cart-, cartlib- of
cartridge-expansie-eigenaar.

| ID | Opdracht | Klaar wanneer |
| --- | --- | --- |
| `STUDIO-FUNCTIONAL-DESIGN-01` | Ontwerp vanuit de echte authoring- en testtaken van de bestaande games en cartlib, na onderzoek van volwassen productievoorbeelden en de live BMSX-representaties. Kies productvolgorde, inspectiemodel, authored representatie, correspondence-scope en productsurfaces zonder een transport of ABI vooruit te ontwerpen. | **Geaccepteerd:** A + C eerst, daarna echte visual authoring; stop-and-inspect, arbitrary Lua source-first, per-node runtimecorrespondence buiten de eerste lens en libretro als speler/core. `STUDIO-BEHAVIOR-RESOURCE-DESIGN-01` heeft de latere constrained authored representatie inmiddels gekozen. Het eisenmodel, de workflows, resolutie-/inputgrenzen en owner-matrix staan in `studio_functional_design.md`. |
| `STUDIO-BEHAVIOR-RESOURCE-DESIGN-01` | Kies de authored representatie vóór een editable graph. Combineer VS Code's text-backed custom-editorlifecycle met afzonderlijke typed resources: BT als ordered tree, FSM als state/transitionresource en ActionEffects als typed collectie. JSONC is canoniek; stable UUID's identificeren authored elementen, terwijl namen, layout en runtime-dense slots dat niet doen. Lua-implementaties blijven expliciete cart-owned bindings die iedere behaviorlibrary eenmaal tijdens admission naar directe closures verlaagt. De packer kookt naar gewone cartdata; machine, TOC, sockets, C++ en cartlib kennen geen Studio. | **Geaccepteerd:** `studio_functional_design.md` specificeert document-, element-, layout-, binding-, cook- en runtimegrenzen met gepinde VS Code, Microsoft JSONC, BehaviorTree.CPP, Godot en Unity-referenties. D en E zijn geen alternatieven meer: het structured resource is een tekstwerkdocument. Geen YAML-roundtrip, generiek graph-DTO, globale callbackregistry, nieuw machine-assettype of automatische Lua-migratie. De eerste productroute is BT en vereist eerst documentowner, schema/cooker en cart-owned admission. |
...
| `BT-AUTHORED-DOCUMENT-01` | Voeg na de documentowner het eerste typed authoringdocument toe: `*.bt.jsonc`. De BT-producer gebruikt Microsoft's JSONC-parser, valideert het versioned ordered-tree-schema met exacte bronranges en kookt naar een afzonderlijk versioned runtimeformaat in een gewone `data`-asset. Authored UUID's identificeren blackboardentries, nodes, decorators en Services; blackboardlinks gebruiken die UUID's en cartcallbacks blijven declaratieve binding-id's. Editor-only id's en namen verdwijnen bij cook, blackboardlinks worden semantische keynamen en de producer schrijft de JSONC-bron nooit terug. | **Geïmplementeerd:** comments en trailing commas worden geaccepteerd; syntax-, property-, type-, enum-, topology-, duplicate-id- en blackboardreferentiefouten dragen offset, lengte, regel en kolom. Tests dekken alle live ingebouwde nodetypen en attachments, inclusief een ingebouwde Wait als Simple-Parallel-hoofd-Task, en bewijzen dat rompack één gewone `data`-asset emit met een bronversie die losstaat van de cooked-formatversie. Het v1-contract staat in `behaviour_tree_resource.md`. Geen editorlayout, gegenereerde Lua-module, runtime-JSON-parser, cartlib-/Studio-import, machine/TOC/C++-type of bronnormalisatie. |
| `BT-RESOURCE-ADMISSION-01` | Voeg daarna aan het BT-domein één cart-owned resource-admissiongrens toe. Zij decodeert de gewone cooked bytes eenmaal via het bestaande contentpad, verifieert cooked format en bindingmanifest, maakt de bestaande blackboardkey-, Task-, Service- en decoratorvormen en geeft daarna de native definitie aan de bestaande BT-library. De cart levert per registratie een expliciet lokaal manifest; admission verlaagt iedere binding-id eenmaal naar de directe callbackdefinitie die de huidige compiler al consumeert. | **Geïmplementeerd:** `cartlib/behaviour_tree/resource.lua` decodeert het gewone adres en bindt typed lokale manifesttabellen voordat zij de native definitie aan de ongewijzigde `library.register`-owner geeft; Lua-authored trees linken daardoor geen decoder/admissioncode. Cart-callbacks kunnen bestaande semantic-keydescriptors delen; declaratieve keys blijven resource-owned. Task-/Service-placementpolicy wordt tegen de concrete binding gecontroleerd en live componenten gebruiken de bestaande compile/install/rebind-owner. De echte `cartlib_test`-ROM kookt een JSONC-tree, importeert uitsluitend `bmsx/assets`, bewijst Task-, Service-, custom- en Blackboard-decoratorbinding, dense Blackboardtoegang, fout-atomische admission en semantic-keybehoud bij vervanging in het guest runtimepad. Geen globale registry, frame-stringlookup, Studio-import, runtime-JSON of machine-/TOC-type. Het manifestcontract staat in `behaviour_tree_resource.md`. |
| `BT-EDITOR-DOCUMENT-PROJECTION-01` | Leg vóór een visual input de afgeleide documentowner vast. De BT-parser indexeert ieder geldig authored element tijdens dezelfde schema-parse op stable id, exacte object-/id-range en JSON path. Eén `BehaviourTreeDocumentModel` per canonical textmodel voegt daar de typed waarden aan toe en bouwt die projectie eenmaal per contentversie. De eerste structured propertycommand schrijft een JSONC-edit via de bestaande textmodel-undo-owner; zij muteert nooit de projectie. | **Geïmplementeerd:** blackboard-, node-, Service- en decoratoridentiteiten hebben directe sourcecorrespondence; twee views van hetzelfde textmodel delen dezelfde retained projectie. Een blackboard-rename verandert alleen het stringliteral, bewaart comments en trailing commas, publiceert één gewone contentevent en loopt door dezelfde undo/redo. Rechtstreekse source-edits en invalid source bouwen de typed projectie deterministisch opnieuw. `EditorTextModel<'behaviour_tree'>` is een echt documenttype maar nog geen code-editorinput; code-editorowners blijven statisch beperkt tot Lua/AEM. Geen UI, whole-document stringify, graphkopie, per-frame parse of runtime-/cartwijziging. |
| `IDE-RESOURCE-EDITOR-RESOLVER-01` | Leg editorselectie vóór de eerste custom visual input bij een workbenchresolver. Een resource behoudt haar machine-/toolchain-type; onafhankelijke editorcontributions selecteren op dat type of een source-suffix en bezitten hun eigen inputopening. Een expliciet editor-id kan later bron- en visual view voor dezelfde resource kiezen. | **Geïmplementeerd:** navigatie kent geen Lua/AEM/viewerbranch meer. De ingebouwde text-editor- en resource-viewercontributions worden eenmaal bij workbenchconstructie gecomponeerd; de resolver kiest specifiek vóór de expliciete wildcard-viewer. Een suffixselector kan `*.bt.jsonc` claimen zonder `editorKind`, BT-type of Studio-metadata op `RuntimeResource`, ROM TOC of machine. De gedeelde textselectie staat niet langer in de code-tab-owner. Geen extension-marketplace-, priority-, configuration- of dynamische registryfacade zolang BMSX die productfuncties niet heeft. |
| `IDE-SOURCE-DATA-CATALOG-01` | Laat de runtime-sourceowner gewone source-backed dataresources indexeren zonder editorclassificatie. Lua blijft uit haar source registry komen; `data` en `aem` komen uit de ROM-index en behouden hun echte assettype. De actieve domain bepaalt welke dataresources zichtbaar zijn, terwijl alle geïnstalleerde domains onder hun `(domain,path)` vindbaar blijven. | **Geïmplementeerd:** de vroegere AEM-only arrays zijn dataresourcecatalogi. Daardoor wordt een cooked `*.bt.jsonc` als gewone `data` vindbaar zonder BT-kennis in machine, TOC of sourceowner. Compiler-`code`, source-loze payloads en andere niet-geselecteerde assetklassen worden niet als textdocument verzonnen. De editorresolver, niet de catalogus, kiest later de visual input. Tests bewijzen system-/slotisolatie en dat de BT-resource haar `data`-representatie behoudt. |
...
| `STUDIO-BT-VISUAL-EDITOR-01` | Bouw pas na documentowner, BT-schema/cooker, BT-admission en `BT-EDITOR-DOCUMENT-PROJECTION-01` een echte host-side BT custom text editor op het canonieke `*.bt.jsonc`-model. De retained projection toont ordered topology, attachments, blackboard en typed properties; alle add/remove/move/connect/propertyacties produceren minimale JSONC-textedits via dezelfde document-undo-owner. Bron- en visual view zijn gelijkwaardige editorinputs op hetzelfde model. | Keyboard, controller en pointer kunnen op 384x288 met het IDE-tiny-font een tree maken en wijzigen; selectie, collapse, viewport en hover blijven viewstate, terwijl iedere semantische of gedeelde-layoutwijziging dirty/save/revert/Hot Resume volgt. Split text/visual fixtures bewijzen bidirectionele synchronisatie, stable-idbehoud bij rename/move en nieuwe UUID's bij copy/paste. Draw parseert, formatteert of alloceert geen topology per frame. Geen React/DOM-workbench, guest viewport, tweede graphdatabase, whole-document stringify, Lua-tablemutatie of observertrace als authored waarheid. |
