...
| `BT-RESOURCE-ADMISSION-01` | Voeg daarna aan de BT-library één cart-owned resource-admissiongrens toe. Zij decodeert de gewone cooked bytes eenmaal via het bestaande contentpad, verifieert cooked format en bindingmanifest, maakt de bestaande blackboardkey-, Task-, Service- en decoratorvormen en voert daarna de bestaande `program.compile` uit. De cart levert per registratie een expliciet lokaal manifest; admission verlaagt iedere binding-id eenmaal naar de directe callbackdefinitie die de huidige compiler al consumeert. | Een echte cartfixture importeert alleen het gewone `bmsx/assets`-adres, registreert een resource vanuit een lokaal bindingsmanifest en draait hetzelfde allocatievrije compiled framepad als een Lua-authored tree. Hot Resume van semantiek of bindingcode kookt/installeert media via de bestaande ROM-owner en rerunt de bestaande annotated cart-init; een incompatibele static-layoutedit wordt eerlijk geweigerd. Geen globale bindingregistry, per-frame stringlookup, Studio-import, generieke behaviorfacade, runtime-JSON-parser of poging om arbitrary Lua te herschrijven. |
