...
| `IDE-SCENE-MEMBER-REMOVE-01` | Voeg na de capture-layoutprerequisite de normale Remove-workbenchactie toe op het huidige directe sourcemember. Deel de analysis-cacheparse; accepteer pending properties vóór sourcecapture, focus na verwijderen documenthistorie en wis verwijderde selectie. | **UI nog niet geleverd; compilerprerequisite nu bewezen:** dezelfde Nemesis-bronverwijdering, Undo/herapply en normale `<init>` werken met dezelfde levende heap op software/WebGL2/WebGPU. De kandidaat-UI moet opnieuw aan de live source-, command-, focus- en selectionowners worden getoetst en de normale pointer/draft/history-route bewijzen. Geen menuactie of featureflag die een ontbrekende owner verbergt; geen live actor-disposal of cartlib-hook. |
