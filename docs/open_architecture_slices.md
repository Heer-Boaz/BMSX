...
| `IDE-GRAPH-VIEWPORT-01` | **Eerstvolgende slice; nog niet gebouwd.** Ontwerp eerst de gedeelde overlay-clipgrens tegen de live `Poly`-, quad-stream- en headless-owners; bouw daarna een retained workbenchcanvas met geometrie, pan, node-/edgeselectie en focus/lifecycle voor de concrete BT-consument. Domeinbetekenis en Lua blijven buiten het control. | Domeinvrije live fixture op software/WebGL2/WebGPU bewijst clipping van gedeeltelijk zichtbare tekst/nodes/edges, dezelfde hitgeometrie, held-pointer paneovergang en geen per-frame layout/topologyallocatie. Eventuele gespiegelde renderwijzigingen hebben vooraf hun representation table en hot-pathcallsites. Geen chrome-afdekvlak, feature-local rasterizer, nieuwe fysieke inputhistorie of generiek extensieframework. |
