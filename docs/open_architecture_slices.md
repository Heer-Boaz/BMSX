...
| `STUDIO-BT-SOURCE-GRAPH-01` | **Eerstvolgende slice; nog niet gebouwd.** Vervang display-afgeleide BT-betekenis bij de bestaande recognizer door typed ordered occurrences/relaties met bronprovenance. Outline en graph gaan dezelfde feiten consumeren; input-owned selectie/collapse volgen bewezen syntaxcorrespondentie via de bestaande text-change-owner, niet naam of row-index. Zie `behavior_graph_design.md`. | Zelfstandige Lua-fixtures bewijzen meerdere registrations per file, hergebruikte subtree-occurrences, volgorde, parallelrollen, weights/attachments, verborgen-pane edits, insert/delete/reorder en incomplete bron. Geen renderer, graphdatabase, runtime-slotidentiteit of cartlibwerk. |
| `IDE-GRAPH-VIEWPORT-01` | Ontwerp eerst de gedeelde overlay-clipgrens tegen de live `Poly`-, quad-stream- en headless-owners; bouw daarna een retained workbenchcanvas met geometrie, pan, node-/edgeselectie en focus/lifecycle voor de concrete BT-consument. Domeinbetekenis en Lua blijven buiten het control. | Domeinvrije live fixture op software/WebGL2/WebGPU bewijst clipping van gedeeltelijk zichtbare tekst/nodes/edges, dezelfde hitgeometrie, held-pointer paneovergang en geen per-frame layout/topologyallocatie. Eventuele gespiegelde renderwijzigingen hebben vooraf hun representation table en hot-pathcallsites. Geen chrome-afdekvlak, feature-local rasterizer, nieuwe fysieke inputhistorie of generiek extensieframework. |
