...
| `IDE-LUA-FULL-FIDELITY-EDIT-DESIGN-01` | Ontwerp structurele Lua-table-edits vanuit complete syntaxbounds, lexicale separators en expliciet trivia-beleid. Toets de vereiste opslag aan de live parser en productievoorbeelden; geen feature-local bronscan of speculatieve CST. | **Verfijnd:** de bestaande `ParsedLuaChunk.tokens` bezit al separators. Complete parser-owned fieldranges plus een keep-exterior-trivia-beleid volstaan voor één bounded verwijderingsprimitive. Insertion/reordering en edits op recovered syntax hebben nog eigen attachment-/error-tree-contracten nodig. `lua_source_syntax_design.md` beschrijft de owner- en kostengrenzen; dit levert nog geen Remove-knop. |
