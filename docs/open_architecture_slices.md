...
| `IDE-SCENE-SOURCE-ADAPTER-01` | Kies na `STUDIO-SCENE-WORKLOAD-01` één bestaande canonical source die een huidige cart aantoonbaar beter kan authoren. Bouw daarop een host-side visual projectie en één echte source-preserving edit via hetzelfde `EditorTextModel`. | De edit verandert een echte cartcompositie, doorloopt undo/redo/save/Hot Resume en behoudt comments, formatting en dynamische Lua-expressies. De adapter voert geen Lua uit, maakt geen tweede database en veronderstelt geen algemene scene-runtime. Een tweede adapter wordt pas generiek wanneer twee productiebronnen dezelfde representatie delen. |
