...
| `IDE-LUA-CAPTURE-LAYOUT-01` | Ontwerp en implementeer lexical capture-correspondence en closure-layoutbehoud/relocatie bij Hot Resume in compiler/linker en zo nodig de gespiegeld aanwezige runtimeowners. Verifieer Roslyn EnC en BLua32-cellidentiteit; debugnaam, toevallige registerindex of hostobjectvorm is geen symbol identity. | **Capture-provenance geleverd, layoutbehoud nog open:** compiler/optimizer/linker en TS/C++ version-5-symbols behouden de definiërende local via een gedeelde indextabel; transitcaptures, shadowing, source maps en tombstones hebben regressiebewijs. Zie `lua_capture_identity_design.md`. Lexical correspondence en pre-lowering slotbehoud ontbreken nog: verwijderen van het echte Nemesis-title-member krimpt `root_scene.register` van zes naar vijf captures en verschuift `director` van slot 5 naar 4; de bestaande guard weigert terecht. Capture contraction/permutation, shadowing, parentregisterwijziging, nested/shared open/closed cells en herhaald remove/undo/reapply werken zonder verkeerde cellen. Echte onmogelijke nieuwe captures blijven expliciet geweigerd. Geen skipped guard, scene-dummy-capture, algemene de-opt, runtime-exception-shortcut of automatische reboot. |
| `IDE-SCENE-MEMBER-REMOVE-01` | Voeg na de capture-layoutprerequisite de normale Remove-workbenchactie toe op het huidige directe sourcemember. Deel de analysis-cacheparse; accepteer pending properties vóór sourcecapture, focus na verwijderen documenthistorie en wis verwijderde selectie. | **Niet geleverd:** de kandidaat-UI doorliep pointer/held-input, draftacceptatie en bron-Undo/Redo, maar faalde echte Save & Hot Resume. Eerst dezelfde Nemesis-verwijdering, Undo/herapply en normale `<init>` met dezelfde levende heap bewijzen op software/WebGL2/WebGPU. Geen menuactie of featureflag die de ontbrekende closure-owner verbergt; geen live actor-disposal of cartlib-hook. |
