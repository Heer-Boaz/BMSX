...
| `CARTLIB-SCENE-COLLECTION-01` | Voeg de gemeten opt-in `cartlib/world/scene_library.lua` toe volgens het ActionEffect-pattern. `register(id, definition)` bewaart de directe definition. `instantiate(id)` loopt haar ordered `{ member_id, definition_id, options }`-records eenmaal af, geeft `options` rechtstreeks aan de bestaande `World:spawn`-owner en retourneert de lokale membermap. | Een echte cartlib-ROM-test bewijst authored order, directe spawn-options, afzonderlijke member-/Registry-identiteit, twee onafhankelijke instanties en dat re-registration alleen toekomstige instantiations wijzigt. `World`, `prefab`, Registry, machine, ROM/TOC en framepaden veranderen niet. Geen validation-/schema-laag, definitionkopie, compilefase, retained instance, unload/reconcile, scene-reference-ABI of Studio-hook. |
