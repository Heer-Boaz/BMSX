...
| `IDE-LUA-TABLE-MOVE-01` | Geef complete field/separatorparen expliciete syntax-owned leading/trailing-trivia volgens Full Moon. Verwissel aangrenzende paren als één language-owned textedit, inclusief een grammaticaal noodzakelijke comma wanneer het separatorloze laatste veld naar voren gaat. | **Geïmplementeerd:** de bestaande lossless lexer draait uitsluitend op de expliciete edit; default AST/tokenopslag blijft gelijk. 9.777 echte corpusbewegingen herparsen met exact de bedoelde AST-volgorde. Tests bewijzen comments/grouping/CRLF, alle fieldvormen, BLua32-evaluatievolgorde, herhaalde beweging en byte-exacte document-Undo/Redo. Geen feature-local scanner, serialisatie of formatter. Referenties en kosten staan in `lua_source_syntax_design.md`. |
