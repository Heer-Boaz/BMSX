...
| `SCENARIO-CALL-ADMISSION-01` | Herstel de aangetoonde setup-call boven een actieve IRQ-continuation. Bestudeer professionele inferior-call-uitvoering en leg TS/C++-representaties en scheduler-callsites vast vóór runtimewijzigingen. | **Gediagnosticeerd, niet opgelost:** cinematic-setup wacht in `dma.wait0_idle` terwijl de onderliggende cart-IRQ niet kan terugkeren. Bewijs vereist voltooide echte cinematic, interrupt/cancel, canonical herstel en geen regressie in Hot Resume/debugger-calls. Geen timeoutverhoging, geforceerde usermode of callsite-guard die op ieder frame opnieuw uitstelt. |
