...
| `IDE-LUA-NUMERIC-LITERAL-EDIT-01` | Voeg bij de generieke Lua-language-owner uitsluitend vervanging van een bestaande signed numeric literal toe. De bewerking gebruikt de bestaande syntaxrange, behoudt de aanwezige decimale/hexadecimale schrijfconventie waar die representatie dat toelaat en levert één `EditorTextEdit` aan het bestaande textmodel. | **Geïmplementeerd:** `ide/language/lua/source_edits.ts` accepteert alleen een bestaande numeric literal of één unary negate met numeric operand, converteert de 1-based inclusieve Lua-range eenmaal naar de textbuffer-range en bewaart decimale padding of hexprefix, width en case. De tests bewijzen één atomic undo/redo-batch over positieve, negatieve, decimale, hex- en exponentbronnen terwijl comments en alle overige bytes exact blijven; een dynamische expressie levert geen edit. Geen scene-/BT-/FSM-type, parserwijziging, source scan, whole-document serializer of parallelle undo-owner. |
...
| `IDE-SCENE-SOURCE-PROJECTION-01` | Projecteer de eerste gekozen canonical source via de bestaande Lua-AST en semantische modulebinding. Herken uitsluitend een immutable import van `cartlib/world/scene_library` en directe `register`-definitions; behoud dynamische objectexpressies expliciet als niet-editable source entries. Deel statische Lua-table-keysemantiek met de language/toolchain-owner. | **Geïmplementeerd:** de workbench-adapter projecteert de echte Nemesis-root als één scene met vier ordered members en directe x/y/z-expressies. Mutable/shadowed modulecalls en computed definitions worden niet als scene aangezien; dynamische arrayentries blijven als `dynamic` zichtbaar en explicit-key velden maken de projectie partial in plaats van ze als ordered runtimeleden te verzinnen. Een echte x-edit loopt van de geprojecteerde AST-expressie via `createLuaIntegerLiteralEdit` en het gedeelde `EditorTextModel`; reparse ziet de nieuwe guestinteger en één undo herstelt de bron exact. Geen Lua-executie, scene-DTO in parser/binder, feature-local table-keyhelper, formatter of tweede working copy. |
