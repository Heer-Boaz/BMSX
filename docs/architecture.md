...
transport-suspension callback run on transitions, with no silent batches or
...
which gates the scheduler while continuing video/UI work, and
[`sound_manager::pause()/resume()`](https://github.com/mamedev/mame/blob/master/src/emu/sound.cpp),
which control output muting independently of sound-device state.
