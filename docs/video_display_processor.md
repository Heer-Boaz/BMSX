...
| Framebuffer execution pass | Host render backend consumes the ready DEX command buffer at the render/presentation boundary. TS headless and native software backends use retained software rasterizers only when the selected backend is explicitly software/headless. WebGL/GLES render the same ordered command stream on the GPU into the backend framebuffer texture and complete with no CPU-readback slot; GPU backends must not route through the software rasterizer or silently fall back to software. Displayed GPU-texture frames remain as a retained device latch; restore replays that latch through the framebuffer execution pass before display presentation. | `src/bmsx/render/backend/software/*`, `src/bmsx/render/backend/webgl/vdp_framebuffer_execution.ts`, `src/bmsx_cpp/render/backend/software/*`, and `src/bmsx_cpp/render/backend/gles2/*`. |
...
- displayed GPU-texture framebuffer command frame;
