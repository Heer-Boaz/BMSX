...
| IDE/terminal framebuffer texture presentation | Host-managed IDE/terminal presentation may display the old framebuffer texture when explicitly enabled by the host overlay mode. Cart and BIOS code do not target this path. | `src/bmsx/render/2d/framebuffer_pipeline.ts`, `src/bmsx_cpp/render/2d/framebuffer_pipeline.cpp`. |
...
- Host framebuffer/RPU execution passes and software framebuffer rasterizers:
  ...
  `src/bmsx/render/backend/webgl/vdp_rpu.ts`,
  `src/bmsx_cpp/render/backend/software/*`,
  `src/bmsx_cpp/render/backend/gles2/vdp_2d_blit.cpp`, and
  `src/bmsx_cpp/render/backend/gles2/vdp_rpu.cpp`
