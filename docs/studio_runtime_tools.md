...
| Frame-context Lua Terminal | installed compiler/debugger binding contract | actual selected binding writes, liveness and frame lifetime, not copied scope tables | open; physical evaluation core and shared firmware name resolution tested on TS/C++, complete lexical coverage/selected-stop admission not exposed; [binding contract](studio_terminal_contexts.md) |
