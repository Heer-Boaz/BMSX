...
| Frame-context Lua Terminal | installed compiler/debugger binding contract | actual selected binding writes, liveness and frame lifetime, not copied scope tables | open; physical/named evaluation tested on TS/C++, pinned-stop admission core tested in Studio; lifetime retirement, full lexical coverage and public/native selection not exposed; [binding contract](studio_terminal_contexts.md) |
