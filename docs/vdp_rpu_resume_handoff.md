...
2. Nieuwe RPU datafiles toegevoegd:
   - `machine/ts/machine/devices/vdp/rpu.ts`
   - `machine/cpp/machine/devices/vdp/rpu.h`
   - `machine/cpp/machine/devices/vdp/rpu.cpp`
3. `VdpDeviceOutput` is versmald naar scanout metadata plus `rpu`:
   - TS: `machine/ts/machine/devices/vdp/device_output.ts`
   - C++: `machine/cpp/machine/devices/vdp/device_output.h`
...
4. Host snapshot schrijft nu alleen de RPU frame pointer/reference door:
   - `machine/ts/render/vdp/view_snapshot.ts`
   - `machine/cpp/render/vdp/view_snapshot.cpp`
...
rg -n "Number\\.isFinite|Number\\.isNaN|isNaN|typeof .*number|Math\\.floor|Math\\.ceil" machine/ts/machine/devices/vdp machine/ts/render/backend/webgl machine/cpp/machine/devices/vdp machine/cpp/render/backend/gles2 machine/firmware/system/vdp_rpu.lua machine/firmware/system/vdp_rpu_quads.lua machine/firmware/system/vdp_xf.lua machine/firmware/system/vdp_lpu.lua carts/bare_metal_cart/cart.lua && exit 1 || true
git diff --check
```
...
Deze eerdere snapshot is niet langer betrouwbaar na de backend-split. Gebruik `git status --short` als actuele bron; de RPU backendfiles staan nu onder `machine/ts/render/backend/webgl/` en `machine/cpp/render/backend/gles2/`, niet onder een gedeelde `render/rpu` map.
