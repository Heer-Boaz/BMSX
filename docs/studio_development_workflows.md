...
| Scenario Lab | Sinds de geïsoleerde [guest-testarchitectuur](guest_testing.md) bezit `workbench/services/testing/scenario_runs.ts` actuele discovery, broncaptatie en run-admission. `TestRun` vraagt per case een eigen fysieke machine; de auteursmachine wordt niet vervangen of hersteld. | Resultaatinspectie is geen interactieve debugger-attachment aan de testmachine. Bronherstel beëindigt oude uitvoering, niet de bruikbaarheid van de workspace-service. |
