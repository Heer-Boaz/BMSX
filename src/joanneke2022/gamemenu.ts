...
export class GameMenu extends WorldObject {
	...
	override paint = (_offset?: vec3): void => {
		...
		$.drawGlyphs(titleX, titleY, titleToDraw, null, null, Msx1Colors[4]);
			...
			default:
				{
					GameMenu.mainItems.forEach(function (item) {
						switch (item.type) {
							case MenuItem.SaveGame:
								$.drawGlyphs(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label, undefined, undefined, Msx1ExtColors[0]);
								break;
							default:
						...
						switch (item.type) {
							case MenuItem.Scale:
								if (!GO.Fullscreen) {
									...
									$.drawGlyphs(offsetX, y, `${$.view.canvasScale.toPrecision(2)}X`);
								}
								else {
									$.drawGlyphs(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label, undefined, undefined, Msx1ExtColors[0]);
