...
export class GameMenu extends mdef implements WorldObject {
	...
	public paint(): void {
		...
		$.drawGlyphs(titleX, titleY, titleToDraw);
		...
		switch (this.CurrentScreen) {
			...
			default:
				{
					GameMenu.mainItems.forEach(function (item) {
						switch (item.type) {
							case MenuItem.SaveGame:
								if ((model as Model).state != GameState.Event)
									$.drawGlyphs(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label);
								else $.drawGlyphs(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label, Msx1ExtColors[0]);
								break;
							default:
								$.drawGlyphs(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label);
								break;
						...
						switch (item.type) {
							case MenuItem.Scale:
								...
								if (!GO.Fullscreen) {
									...
									$.drawGlyphs(offsetX, y, `${view.scale.toPrecision(2)}X`);
								}
								else {
									$.drawGlyphs(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label, Msx1ExtColors[0]);
									offsetX += GameMenu.scaleText.length * TextWriter.FontWidth;
			...
			case MenuItem.LoadFromMainMenu:
				{
					$.drawGlyphs(GameMenu.menuPosX + GameMenu.loadsaveItemOffsetX, y, GameMenu.backText);
					y += GameMenu.itemOffsetY;
	...
	private printSaveSlot(x: number, y: number, slotIndex: number): void {
		...
		if (!exists) {
			$.drawGlyphs(GameMenu.menuPosX + GameMenu.loadsaveItemOffsetX, y, `${slotIndex} + 1: ${GameMenu.emptySlot}`);
			return;
		}
		let savegame = LoadGame(slotIndex);
		let time = savegame.Timestamp;
		$.drawGlyphs(GameMenu.menuPosX + GameMenu.loadsaveItemOffsetX, y, `${slotIndex + 1}: ${time.getDay().toFixed(2)}/${time.getMonth().toFixed(2)}/${time.getFullYear().toFixed(2)} - ${time.getHours().toFixed(2)}:${time.getMinutes().toFixed(2)}`);
	}
}
