		...
		switch (this.state) {
			...
			case State.SelectMain:
				for (let i = 0; i < MainMenu.items.length; i++) {
					switch (MainMenu.menuOptions[i]) {
						case MenuItem.Continue:
							if (SlotExists(CS.SaveSlotCheckpoint))
								$.drawGlyphs(MainMenu.itemsX, MainMenu.itemYs[i], MainMenu.items[i]);
							else $.drawGlyphs(MainMenu.itemsX, MainMenu.itemYs[i], MainMenu.items[i], Msx1Colors[0]);
							break;
						default:
							$.drawGlyphs(MainMenu.itemsX, MainMenu.itemYs[i], MainMenu.items[i]);
							break;
			...
			case State.SelectChapter:
				for (let i = 0; i < MainMenu.chapterItems.length; i++) {
					$.drawGlyphs(MainMenu.itemsX, MainMenu.itemYs[i], MainMenu.chapterItems[i]);
				}
				break;
		}
