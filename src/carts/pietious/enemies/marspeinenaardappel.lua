...
local enemy_def_id = string.format('%s.%s', constants.ids.enemy_def, 'marspeinenaardappel')
...
function marspeinenaardappel:bind_overlap_events()
	self.events:on({
		event_name = 'overlap',
		subscriber = self,
...
function marspeinenaardappel:move_with_velocity()
...
function marspeinenaardappel:take_weapon_hit(weapon_kind, hit_id)
...
function marspeinenaardappel:on_overlap(event)
...
marspeinenaardappel.enemy_def_id = enemy_def_id
