...
local enemy_def_id = string.format('%s.%s', constants.ids.enemy_def, 'paperfoe')

function paperfoe:configure_from_room_def(def, room)
...
function paperfoe:bind_overlap_events()
	self.events:on({
		event_name = 'overlap',
		subscriber = self,
...
function paperfoe:spawn_child_enemy(kind, x, y, options)
	options = options or {}
...
function paperfoe:projectile_is_out_of_bounds()
...
function paperfoe:set_active_behaviour_tree(kind)
...
function paperfoe:set_velocity(speed_x_num, speed_y_num, speed_den)
...
function paperfoe:move_with_velocity()
...
function paperfoe:take_weapon_hit(weapon_kind, hit_id)
...
function paperfoe:on_overlap(event)
...
paperfoe.enemy_def_id = enemy_def_id
