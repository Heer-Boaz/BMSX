...
function staffspawn:bind_overlap_events()
	self.events:on({
		event_name = 'overlap',
		subscriber = self,
...
function staffspawn:projectile_is_out_of_bounds()
...
function staffspawn:set_velocity(speed_x_num, speed_y_num, speed_den)
...
function staffspawn:spawn_death_effect()
...
function staffspawn:take_weapon_hit(weapon_kind, hit_id)
...
function staffspawn:on_overlap(event)
	...
	if other_collider.id_local == constants.ids.player_body_collider_local and self.dangerous then
		player:take_hit(self.damage, self.x + math.modf(self.sx / 2), self.y + math.modf(self.sy / 2), 'staffspawn')
	end
end
