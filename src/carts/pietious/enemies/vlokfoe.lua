...
local enemy_def_id = string.format('%s.%s', constants.ids.enemy_def, 'vlokfoe')

function vlokfoe:configure_from_room_def(def, room)
...
function vlokfoe:bind_overlap_events()
	self.events:on({
		event_name = 'overlap',
		subscriber = self,
...
function vlokfoe:spawn_child_enemy(kind, x, y, options)
	options = options or {}
...
function vlokfoe:projectile_is_out_of_bounds()
...
function vlokfoe:set_active_behaviour_tree(kind)
...
function vlokfoe:set_velocity(speed_x_num, speed_y_num, speed_den)
...
function vlokfoe:move_with_velocity()
...
function vlokfoe:take_weapon_hit(weapon_kind, hit_id)
...
function vlokfoe:on_overlap(event)
...
vlokfoe.enemy_def_id = enemy_def_id
