...
local enemy_def_id = string.format('%s.%s', constants.ids.enemy_def, 'mijterfoe')
...
function mijterfoe:spawn_child_enemy(kind, x, y, options)
	options = options or {}
...
function mijterfoe:set_active_behaviour_tree(kind)
...
mijterfoe.enemy_def_id = enemy_def_id
