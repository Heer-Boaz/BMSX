...
function fighter:sample_player_input()
	...
	mem[sys_inp_query] = string_ref(constants.controls.left)
	self.left_held = mem[sys_inp_status] ~= 0
	mem[sys_inp_query] = string_ref(constants.controls.right)
	self.right_held = mem[sys_inp_status] ~= 0
	mem[sys_inp_query] = string_ref(constants.controls.jump)
	self.jump_held = mem[sys_inp_status] ~= 0
	self.jump_pressed = self.jump_held and not was_jump_held
	mem[sys_inp_query] = string_ref(constants.controls.punch)
	self.attack_a_pressed = mem[sys_inp_status] ~= 0
	mem[sys_inp_query] = string_ref(constants.controls.kick)
