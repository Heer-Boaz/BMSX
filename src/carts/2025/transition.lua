...
function transition.register_states(states)
	...
	local build_transition_palette<const> = function(style)
		...
		local setup_overlay<const> = function()
	...
	local hide_transition_panels<const> = function()
	...
	local build_transition_layout<const> = function(style, palette, layout)
		...
		local configure_panel<const> = function(panel)
			local sprite<const> = object(panel.id)
			sprite.visible = true
	...
	states.transition = {
		entering_state = function(self)
			...
			reset_text_colors()
			local transition_text<const> = object(text_transition_id)
			...
			local overlay<const> = setup_overlay()
			...
			local accent<const> = object(self.transition_accent.id)
