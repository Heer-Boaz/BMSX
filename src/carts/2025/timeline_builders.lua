...
local flash_mix<const> = function(frame_index, flash_frame)
	...
	local u<const> = (frame_index - flash_frame) / (transition_flash_frames - 1)
...
function builders.build_combat_focus_frames(params)
	...
	local zoom_scale<const> = combat_focus_zoom_scale
	local zoom_target_x<const> = (display_width() - (monster_sx * zoom_scale)) / 2
	local zoom_target_y<const> = (display_height() - (monster_sy * zoom_scale)) / 2

	local vanish_scale_x<const> = combat_focus_vanish_scale_x
	local vanish_scale_y<const> = combat_focus_vanish_scale_y
	local vanish_center_x<const> = display_width() / 2
	local vanish_bottom_y<const> = zoom_target_y + (monster_sy * zoom_scale)

	for i = 0, combat_focus_zoom_frames - 1 do
		local u<const> = i / (combat_focus_zoom_frames - 1)
		local eased<const> = easing.smoothstep(u)
		local turn<const> = easing.arc01(u)
		local s<const> = 1 + ((zoom_scale - 1) * eased)
	...
	for i = 0, combat_focus_vanish_frames - 1 do
		...
		local turn<const> = easing.arc01(u)
		local sx<const> = zoom_scale + ((vanish_scale_x - zoom_scale) * melt)
		local sy<const> = zoom_scale + ((vanish_scale_y - zoom_scale) * melt)
	...
	local monster_hidden_y<const> = monster_start_y - monster_start_oy
		...
		local maya_a_hidden_y<const> = maya_a_base_y - (maya_a_sy * (maya_a_start_scale - 1))
			local hold_frames<const> = combat_intro_hold_frames
			local maya_b_motion_frames<const> = combat_intro_maya_b_frames - hold_frames
...
function builders.build_combat_dodge_frames(params)
	...
	local base_x<const> = params.base_x
	local anticipate_frames<const> = combat_dodge_anticipation_frames
	local peak_frames<const> = combat_dodge_peak_frames
	local recover_frames<const> = combat_dodge_recover_frames
	local move_frames<const> = combat_dodge_frame_count - anticipate_frames - peak_frames - recover_frames
	local move_end<const> = anticipate_frames + move_frames
	local peak_end<const> = move_end + peak_frames
	...
	for frame_index = 0, combat_dodge_frame_count - 1 do
		...
		if frame_index < anticipate_frames then
			local u<const> = frame_index / (anticipate_frames - 1)
		...
		elseif frame_index < move_end then
			local u<const> = (frame_index - anticipate_frames) / (move_frames - 1)
		...
		elseif frame_index < peak_end then
			...
			local u<const> = (frame_index - peak_end) / (recover_frames - 1)
...
function builders.build_combat_exchange_frames(params)
	...
	local anticipate_frames<const> = combat_exchange_anticipate_frames
	local lunge_frames<const> = combat_exchange_lunge_frames
	local hitstop_frames<const> = combat_exchange_hitstop_frames
	local recover_frames<const> = frame_count - anticipate_frames - lunge_frames - hitstop_frames
	local lunge_end<const> = anticipate_frames + lunge_frames
	local hitstop_end<const> = lunge_end + hitstop_frames
	...
	for i = 0, frame_count - 1 do
		...
		if i < anticipate_frames then
			local u<const> = i / (anticipate_frames - 1)
			lunge = -0.10 * easing.smoothstep(u)
		elseif i < lunge_end then
			local u<const> = (i - anticipate_frames) / (lunge_frames - 1)
...
function builders.build_combat_hit_frames(params)
	...
	local monster_sx<const> = params.monster_sx
	local monster_sy<const> = params.monster_sy
	local hold_in<const> = combat_hit_stop_frames
	local peak_frames<const> = combat_hit_peak_frames
	local recover_frames<const> = combat_hit_recover_frames
	local move_frames<const> = combat_hit_frame_count - hold_in - peak_frames - recover_frames
	local peak_start<const> = hold_in + move_frames
	local recover_start<const> = peak_start + peak_frames
	local slash_start<const> = hold_in
	...
	for frame_index = 0, combat_hit_frame_count - 1 do
		...
		if frame_index >= hold_in and frame_index < peak_start then
			local u<const> = (frame_index - hold_in) / (move_frames - 1)
		...
		elseif frame_index >= recover_start then
			local u<const> = (frame_index - recover_start) / (recover_frames - 1)
		...
		if frame_index >= hold_in and frame_index < (hold_in + combat_hit_shake_frames) then
			local k<const> = frame_index - hold_in
		...
		if frame_index >= hold_in and frame_index < recover_start then
			local flash_index<const> = frame_index - hold_in
			if (flash_index % 2) == 1 then
				monster_colorize = { r = 1, g = 0.2, b = 0.2, a = 1 }
			end
		end

		local slash_active<const> = frame_index >= slash_start and frame_index <= slash_end
		local slash_points = { 0, 0, 0, 0 }
		local slash_thickness = 0
		local slash_color = { r = 1, g = 1, b = 1, a = 0 }
		if slash_active then
			local u<const> = (frame_index - slash_start) / (slash_end - slash_start)
