...
export class MuzzleFlash extends PooledGameObject {
    ...
    override paint(): void { if (!this.active) return; submitParticle({ position: [this.p.x, this.p.y, this.p.z], size: this.p.size, color: { r: this.p.r, g: this.p.g, b: this.p.b, a: this.p.a } }); }
}
...
export class ImpactBurst extends PooledGameObject {
    ...
    override paint(): void { if (!this.active) return; for (const p of this.ps) { if (p.life < p.max) submitParticle({ position: [p.x, p.y, p.z], size: p.size, color: { r: p.r, g: p.g, b: p.b, a: p.a } }); } }
}
