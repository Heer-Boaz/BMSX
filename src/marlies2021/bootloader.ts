...
class speler extends SpriteObject {
	...
	override paint = (offset?: vec2, colorize?: { r: boolean, g: boolean, b: boolean, a: boolean; }) => {
		if (this.state.getCurrentId() == 'spot') {
			drawText(64, 40, "Artiestieke impressie");
			drawText(64, 48, "van stressoren zoals");
			drawText(64, 56, "corona,moederzorgen,");
			drawText(64, 64, "voetbal en f1.");

			_game.view.drawImg(BitmapId.arrowed, 0, 32, DrawImgFlags.None);
		}
		if (this.state.getCurrentId() == 'spot2') {
			drawText(64, 40, "Druk de shifttoets");
			drawText(64, 48, "om meditatietechnieken");
			drawText(64, 56, "toe te passen tegen,");
			drawText(64, 64, "de stressoren!");
			drawText(64, 72, "Beweeg met");
			drawText(64, 80, "links en rechts!");
		}
...
_global['h406A'] = (args: BootArgs): void => {
	...
	if (!platformServices) {
		throw new Error('[Bootloader:marlies2021] Platform services not provided. Ensure the host injects PlatformServices before starting the game.');
	}
	let _view = new viewclass(new_vec2(MSX1ScreenWidth, MSX1ScreenHeight));
	_model = new modelclass();
	new Game({ ...args, model: _model, view: _view }).init({ ...args, platformServices, _view }).then((game) => {
