...
import { drawText } from "bmsx/render/text";
...
export class GameMenu extends WorldObject {
	...
	override paint = (offset?: vec3):void => {
		...
		drawText(titleX, titleY, titleToDraw);
		...
		switch (this.CurrentScreen) {
			...
			default:
				{
					GameMenu.mainItems.forEach(function (item) {
						switch (item.type) {
							case MenuItem.SaveGame:
								// if ((model as Model).state != GameState.Event)
									// drawText(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label);
								// else
								drawText(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label, null, Msx1ExtColors[0]);
								break;
							default:
								drawText(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label);
								break;
						...
						switch (item.type) {
							case MenuItem.Scale:
								...
								if (!GO.Fullscreen) {
									...
									drawText(offsetX, y, `${game.view.scale.toPrecision(2)}X`);
								}
								else {
									drawText(GameMenu.menuPosX + GameMenu.mainItemsOffsetX, y, item.label, null, Msx1ExtColors[0]);
			...
			case MenuItem.LoadFromMainMenu:
				{
					drawText(GameMenu.menuPosX + GameMenu.loadsaveItemOffsetX, y, GameMenu.backText);
					y += GameMenu.itemOffsetY;
	...
	private printSaveSlot(x: number, y: number, slotIndex: number): void {
		...
		// if (!exists) {
		//     drawText(GameMenu.menuPosX + GameMenu.loadsaveItemOffsetX, y, `${slotIndex} + 1: ${GameMenu.emptySlot}`);
		//     return;
		// }
		// let savegame = LoadGame(slotIndex);
		// let time = savegame.Timestamp;
		// drawText(GameMenu.menuPosX + GameMenu.loadsaveItemOffsetX, y, `${slotIndex + 1}: ${time.getDay().toFixed(2)}/${time.getMonth().toFixed(2)}/${time.getFullYear().toFixed(2)} - ${time.getHours().toFixed(2)}:${time.getMinutes().toFixed(2)}`);
	}
}
