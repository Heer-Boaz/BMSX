...
import { createLuaDemoWorldModule } from './worldmodule';
...
globalTarget.h406A = async (args: BootArgs): Promise<void> => {
	...
	if (!platform) {
		throw new Error('[Bootloader:luademo] Platform instance not provided.');
	}

	const viewHost = args.viewHost ?? platform.gameviewHost;
	if (!viewHost) {
		throw new Error('[Bootloader:luademo] View host not provided by platform.');
	}

	const module = createLuaDemoWorldModule();
	const worldConfiguration: WorldConfiguration = {
		viewportSize: { x: 128, y: 128 },
		viewCanvasSize: { x: 128, y: 128 },
		viewOffscreenSize: { x: 128, y: 128 },
		modules: [module],
	};
