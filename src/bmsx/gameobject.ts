...
export class GameObject implements vec2, vec3, IComponentContainer, IIdentifiable {
    ...
    public hitarea: Area;
    public boundingBoxes: Area[]; // DARN: ON CREATING SAVEGAME, THIS WILL BE SAVED AND THEN LOADED IN THE GAMEOBJECT. THIS IS NOT WHAT WE WANT. WE WANT TO LOAD THE BOUNDING BOXES FROM THE IMG ASSET. THIS IS A TEMPORARY FIX AND THIS COMMENT IS GENERATED BY AICOPILOT, WHICH MEANS I AM NOT THE ONLY ONE FACING THIS ISSUE :-)
    ...
    */
    ...
    public collides(o: GameObject | Area): boolean {
        ...
        if ((o as GameObject).id) { // If the given object has an id, it is a GameObject
            ...
            if (!this.detect_aabb_collision_area(other.hitarea)) {
                return false;
            }

            if (this.boundingBoxes && other.boundingBoxes) { // If both objects have bounding boxes, check if any of the bounding boxes overlap
                for (const box1 of this.boundingBoxes) { // Iterate over the bounding boxes of this object
                    for (const box2 of other.boundingBoxes) { // Iterate over the bounding boxes of the other object
                        if (this.detect_aabb_collision_area(box1) && other.detect_aabb_collision_area(box2)) { // If the bounding boxes overlap, return true
                            return true;
                        }
                    }
                }
                return false; // If none of the bounding boxes overlap, return false
            } else if (this.boundingBoxes) { // If only this object has bounding boxes, check if any of the bounding boxes overlap with the other object
                ...
                return false; // If none of the bounding boxes overlap, return false
            } else if (other.boundingBoxes) { // If only the other object has bounding boxes, check if any of the bounding boxes overlap with this object
                ...
                return this.detect_object_collision(other); // If neither object has bounding boxes, check if the objects overlap
            }
        } else {
            // Broad-phase collision detection: AABB test
            ...
            if (this.boundingBoxes) { // If this object has bounding boxes, check if any of the bounding boxes overlap with the given area
                for (const box of this.boundingBoxes) { // Iterate over the bounding boxes of this object
                ...
                return this.detect_aabb_collision_area(o as Area); // If this object does not have bounding boxes and the other object is an area, check if the given area overlaps with this object
            }
        }
    }
    ...
    public static detect_aabb_collision_areas(a1: Area, a2: Area): boolean {
        return !(a1.start.x > a2.end.x || a2.end.x < a2.start.x || a1.end.y < a2.start.y || a1.start.y > a2.end.y);
    }
