...
export class PendingAssignmentProcessor {
	...
	private notifyUIProposed(gamepadIndex: number, proposedPlayerIndex: number | null): void {
		$.emit_presentation('controller_assignment_proposed', Input.instance, { gamepadIndex, proposedPlayerIndex });
	}

	private notifyUIAssignmentStart(gamepadIndex: number, proposedPlayerIndex: number | null): void {
		$.emit_presentation('controller_assignment_start', Input.instance, { gamepadIndex, proposedPlayerIndex });
	}
	...
	async run(): Promise<void> {
		...
		if (this.proposedPlayerIndex === null) {
			...
			if (this.checkNonConsumedPressed('a', gamepadInput)) {
				...
				$.emit_presentation('controller_assigned', Input.instance, { proposedPlayerIndex: this.proposedPlayerIndex, gamepadIndex: gamepadInput.gamepadIndex });
			}
				...
				$.emit_presentation('controller_assignment_cancelled', Input.instance, { proposedPlayerIndex: this.proposedPlayerIndex, gamepadIndex: gamepadInput.gamepadIndex });
			}
			else {
