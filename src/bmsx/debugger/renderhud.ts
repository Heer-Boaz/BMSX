...
export class RenderHUDOverlay implements RegisterablePersistent {
	...
	updateNow(): void {
		...
		if (this.useEMA) {
			...
			lines.push(`Frame ${Math.floor($.platform.clock.now())} time:${total.toFixed(2)}ms avg:${this.emaFrameAvg.toFixed(2)}ms mode=${modeStr}`);
			...
			lines.push(`Frame ${Math.floor($.platform.clock.now())} time:${total.toFixed(2)}ms avg:${frameAvg.toFixed(2)}ms mode=${modeStr}`);
