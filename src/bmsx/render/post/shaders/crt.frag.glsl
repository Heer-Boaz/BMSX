#version 300 es
precision highp float;

// --- Textures & core uniforms ---
uniform sampler2D u_texture;          // offscreen scene (size = u_srcResolution * u_fragscale)
uniform vec2 u_srcResolution;        // base "logical" resolution (e.g., 256x212)
uniform float u_fragscale;            // integer upscale (e.g., 2.0)

// Time/random for noise
uniform float u_time;
uniform float u_random;

// --- Feature toggles ---
uniform bool u_applyNoise;
uniform bool u_applyColorBleed;
uniform bool u_applyScanlines;
uniform bool u_applyBlur;
uniform bool u_applyGlow;
uniform bool u_applyFringing;
// uniform bool u_applyApertureMask;

// --- Parameters ---
uniform float u_noiseIntensity;      // 0..~0.5
uniform vec3 u_colorBleed;          // small additive bias, linear space
uniform float u_blurIntensity;       // 0..1 blend
// uniform float u_blurFootprintPx;     // kernel step in source px (suggest 0.5..0.8)
uniform vec3 u_glowColor;           // glow tint (linear)

// ---- Constants ----
const vec3 LUMA = vec3(0.299, 0.587, 0.114);

// scanlines & optics
const float SCANLINE_INTERVAL = 1.0;    // one source row
const float SIGMA_DARK = 0.28;
const float SIGMA_BRIGHT = 0.60;

const float APERTURE_STRENGTH = 0.08;
const float GLOW_BRIGHTNESS_CLAMP = 0.6;

// fringing (chromatic aberration)
const float FRINGING_BASE_PX = 0.8;
const float FRINGING_QUAD_COEF = 2.5;
const float FRINGING_CONTRAST_COEF = 0.4;
const float FRINGING_MIX = 0.22;

// 5x5 binomial-ish kernel (UN-normalized). Multiply by K_NORM per tap.
const float K_NORM = 1.0 / 256.0;
const float KERNEL_5x5[25] = float[](1.0, 4.0, 6.0, 4.0, 1.0, 4.0, 16.0, 24.0, 16.0, 4.0, 6.0, 24.0, 36.0, 24.0, 6.0, 4.0, 16.0, 24.0, 16.0, 4.0, 1.0, 4.0, 6.0, 4.0, 1.0);

in vec2 v_texcoord;
out vec4 outputColor;

// --- Helpers: gamma/linear ---
vec3 toLinear(vec3 c) {
    return pow(c, vec3(2.2));
}
vec3 toSRGB(vec3 c) {
    return pow(max(c, 0.0), vec3(1.0 / 2.2));
}

// --- Noise ---
float hashNoise(vec2 uv, float t) {
    vec3 p = vec3(uv * 0.1, t * 0.1);
    p = fract(p * vec3(12.9898, 78.233, 43758.5453));
    p += dot(p, p.yzx + 19.19);
    return fract((p.x + p.y) * p.z);
}

// --- Blur + local contrast around center tap ---
struct BlurContrast {
    vec3 blurred;
    float contrast;
};

BlurContrast applyBlurAndContrast(vec2 uv, vec2 texel, float footprintPx) {
    vec2 stepUV = texel * footprintPx; // shrink/expand kernel footprint in source px
    vec3 accum = vec3(0.0);
    float centerLum = 0.0;
    float neighLum = 0.0;
    float neighCount = 0.0;

    int r = 2;
    int idx = 0;
    for (int y = -r; y <= r; ++y) {
        for (int x = -r; x <= r; ++x, ++idx) {
            vec2 ofs = vec2(float(x), float(y)) * stepUV;
            vec3 s = toLinear(texture(u_texture, uv + ofs).rgb);
            float w = KERNEL_5x5[idx] * K_NORM;   // <<< normalized per tap
            accum += s * w;

            if (abs(x) <= 1 && abs(y) <= 1) {
                float lum = dot(s, LUMA);
                if (x == 0 && y == 0)
                    centerLum = lum;
                else {
                    neighLum += lum;
                    neighCount += 1.0;
                }
            }
        }
    }
    float neighAvg = (neighCount > 0.0) ? (neighLum / neighCount) : centerLum;
    return BlurContrast(accum, abs(centerLum - neighAvg));
}

// --- Scanlines in source-pixel space ---
vec3 applyScanlines(vec3 colorLinear, vec2 uv, vec2 srcPxRes) {
    float y_src = uv.y * srcPxRes.y;
    float lum = dot(colorLinear, LUMA);
    float sigma = mix(SCANLINE_INTERVAL * SIGMA_DARK, SCANLINE_INTERVAL * SIGMA_BRIGHT, clamp(lum, 0.0, 1.0));
    float scanPos = mod(y_src + 0.5, SCANLINE_INTERVAL) - 0.5 * SCANLINE_INTERVAL;
    float g = exp(-(scanPos * scanPos) / (2.0 * sigma * sigma));
    g = pow(g, 0.8);
    return colorLinear * g;
}

// --- Aperture grille ---
vec3 applyApertureMask(vec3 colorLinear, vec2 uv, vec2 srcPxRes) {
    float x_src = uv.x * srcPxRes.x;
    float triad = 0.5 + 0.5 * cos(6.2831853 * x_src);
    vec3 mask = vec3(1.0 + APERTURE_STRENGTH * triad, 1.0, 1.0 - APERTURE_STRENGTH * triad);
    return colorLinear * mask;
}

void main() {
    // pixel geometry
    vec2 srcPxRes = u_srcResolution * u_fragscale; // offscreen size in source px
    vec2 texel = 1.0 / srcPxRes;

    // base sample (sRGB -> linear)
    vec3 color = toLinear(texture(u_texture, v_texcoord).rgb);

    // 1) signal tweak
    if (u_applyColorBleed) {
        color += u_colorBleed;
    }

    // 2) beam spread (blur) — do this BEFORE scanlines/lens to keep them crisp
    BlurContrast bc;
    if (u_applyBlur || u_applyFringing) {
        // float fp = (u_blurFootprintPx > 0.0) ? u_blurFootprintPx : 0.75; // tighten if needed
        bc = applyBlurAndContrast(v_texcoord, texel, 0.75);
    } else {
        bc.blurred = color;
        bc.contrast = 0.0;
    }
    if (u_applyBlur) {
        color = mix(color, bc.blurred, clamp(u_blurIntensity, 0.0, 1.0));
    }

    // 3) lens (chromatic aberration / fringing)
    if (u_applyFringing) {
        vec2 dUV = v_texcoord - 0.5;
        float d = length(dUV) / length(vec2(0.5));
        vec2 dir = (d > 0.0) ? (dUV / max(d, 1e-6)) : vec2(1.0, 0.0);

        float shiftPx = FRINGING_BASE_PX + FRINGING_QUAD_COEF * (d * d) + FRINGING_CONTRAST_COEF * bc.contrast;

        vec2 shiftUV = dir * (shiftPx * texel);

        float r = texture(u_texture, v_texcoord + shiftUV).r;
        float g = texture(u_texture, v_texcoord).g;
        float b = texture(u_texture, v_texcoord - shiftUV).b;
        vec3 fringed = toLinear(vec3(r, g, b));

        color = mix(color, fringed, FRINGING_MIX);
    }

    // 4) beam modulation (scanlines)
    if (u_applyScanlines) {
        color = applyScanlines(color, v_texcoord, srcPxRes);
    }

    // 5) shadow mask / aperture grille
    // if (u_applyApertureMask) {
    color = applyApertureMask(color, v_texcoord, srcPxRes);
    // }

    // 6) phosphor glow
    if (u_applyGlow) {
        float brightness = dot(color, LUMA);
        color += u_glowColor * clamp(brightness, 0.0, GLOW_BRIGHTNESS_CLAMP);
    }

    // 7) analog/RF noise last
    if (u_applyNoise) {
        float y_src = v_texcoord.y * srcPxRes.y;
        float lineNoise = hashNoise(vec2(0.0, floor(y_src) + u_time * 30.0), 0.0) - 0.5;
        float pixNoise = hashNoise(v_texcoord * srcPxRes + vec2(u_random), u_time) - 0.5;
        float lum = dot(color, LUMA);
        float n = mix(pixNoise, lineNoise, 0.35);
        color += color * n * (u_noiseIntensity * mix(0.2, 1.0, 1.0 - lum));
    }

    color = clamp(color, 0.0, 1.2);
    outputColor = vec4(clamp(toSRGB(color), 0.0, 1.0), 1.0);
}
