...
export function commitVdpSkyboxViewState(view: GameView, vdp: VDP, assets: RuntimeAssetState): void {
	...
	for (let index = 0; index < SKYBOX_FACE_KEYS.length; index += 1) {
		...
		const sample = vdp.resolveBlitterSample(resolveImageSlotSource(assets, assetId));
