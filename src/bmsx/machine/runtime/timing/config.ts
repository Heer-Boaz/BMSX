...
export function refreshDeviceTimings(nowCycles: number): void {
...
export function setCycleBudgetPerFrame(value: number): void {
	const runtime = Runtime.instance;
	...
	refreshDeviceTimings(runtime.machine.scheduler.currentNowCycles());
	runtime.vblank.configureCycleBudget();
}

export function setFrameTiming(cpuHz: number, cycleBudgetPerFrame: number, vblankCycles: number): void {
	const runtime = Runtime.instance;
	const timing = runtime.timing;
	timing.cpuHz = cpuHz;
...
function setRenderWorkUnitsPerSec(vdpValue: number, geoValue: number): void {
	const runtime = Runtime.instance;
...
export function applyActiveMachineTiming(cpuHz: number): void {
	const runtime = Runtime.instance;
	...
	setRenderWorkUnitsPerSec(perfSpecs.work_units_per_sec, perfSpecs.geo_work_units_per_sec);
}

export function setTransferRatesFromManifest(specs: TransferRateManifest): void {
	const runtime = Runtime.instance;
