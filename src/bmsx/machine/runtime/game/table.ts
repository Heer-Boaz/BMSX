...
import { packActionStateFlags } from '../../firmware/input_state_tables';
...
export function createRuntimeGameTable(): Table {
	...
	const clockPerfNowFn = createNativeFunction('platform.clock.perf_now', (_args, out) => {
		out.push(runtime.clock.perf_now());
	});
	...
	const getActionStateFn = createNativeFunction('game.get_action_state', (args, out) => {
		...
		if (typeof playerOrAction === 'number') {
			...
			out.push(packActionStateFlags(Input.instance.getPlayerInput(playerOrAction).getActionState(action.text, windowFrames)));
			return;
		}
		const windowFrames = args.length > 1 && args[1] !== null ? args[1] as number : undefined;
		out.push(packActionStateFlags(Input.instance.getPlayerInput(1).getActionState(playerOrAction.text, windowFrames)));
	});
