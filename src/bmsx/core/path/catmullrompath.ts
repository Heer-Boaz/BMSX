...
export class CatmullRomPath extends EventEmitter implements IPath {
    points: PathPoint[] = [];
    length = 0;
    ...
    static fromJSON(json: string | CatmullRomPathJSON): CatmullRomPath { const obj = (typeof json === 'string') ? JSON.parse(json) : json; const pts: PathPoint[] = obj.points?.map(p => ({ p: { x: p.x, y: p.y, z: p.z }, t: p.t, meta: p.meta })) ?? []; return new CatmullRomPath(pts); }
