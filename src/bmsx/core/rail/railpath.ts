...
export class RailRunner { u = 0; speed = 0.1; rail: RailPath; looping = false; paused = false; constructor(rail: RailPath) { this.rail = rail; } update(dt: number) { if (this.paused) return; const prev = this.u; this.u += this.speed * dt; if (this.u > 1) { if (this.looping) this.u = this.u % 1; else this.u = 1; } this.rail.updateAndFire(prev, this.u); } sample(): RailSample { return this.rail.sample(this.u); } }
