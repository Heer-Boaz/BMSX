...
export class Runtime {
	...
	private static resolveCpuHz(machineConfig: { cpu_freq_hz?: number }): number {
		const value = machineConfig.cpu_freq_hz;
		if (value === undefined) {
			throw new Error('[Runtime] machine.cpu_freq_hz is required.');
		}
		if (!Number.isSafeInteger(value) || value <= 0) {
			throw new Error('[Runtime] machine.cpu_freq_hz must be a positive safe integer.');
		}
		return value;
	}

	private static resolveBytesPerSec(machineConfig: { [key: string]: number | undefined }, key: string): number {
		const value = machineConfig[key];
		if (value === undefined) {
			throw new Error(`[Runtime] machine.${key} is required.`);
		}
		if (!Number.isSafeInteger(value) || value <= 0) {
			throw new Error(`[Runtime] machine.${key} must be a positive safe integer.`);
		}
		return value;
	}

	private static resolveUfpsScaled(machineConfig: { ufps?: number }): number {
		const value = machineConfig.ufps;
		if (value === undefined) {
			throw new Error('[Runtime] machine.ufps is required.');
		}
		if (!Number.isSafeInteger(value) || value <= 0) {
			throw new Error('[Runtime] machine.ufps must be a positive safe integer.');
		}
		return value;
	}

	private static applyUfpsScaled(machineConfig: { ufps?: number }): number {
		const ufpsScaled = Runtime.resolveUfpsScaled(machineConfig);
	...
	private setTransferRatesFromManifest(machineConfig: { imgdec_bytes_per_sec?: number; dma_bytes_per_sec_iso?: number; dma_bytes_per_sec_bulk?: number }): void {
		this.imgDecBytesPerSec = Runtime.resolveBytesPerSec(machineConfig, 'imgdec_bytes_per_sec');
		this.dmaBytesPerSecIso = Runtime.resolveBytesPerSec(machineConfig, 'dma_bytes_per_sec_iso');
		this.dmaBytesPerSecBulk = Runtime.resolveBytesPerSec(machineConfig, 'dma_bytes_per_sec_bulk');
	...
	public static async init(cartridge?: Uint8Array): Promise<void> {
		...
		if (!cartridge) {
			...
			const engineMemoryLimits = Runtime.resolveMemoryMapLimits({
			...
			Runtime.applyUfpsScaled(engineLayer.index.manifest.machine);
			const cpuHz = Runtime.resolveCpuHz(engineLayer.index.manifest.machine);
			...
			runtime.setTransferRatesFromManifest(engineLayer.index.manifest.machine);
		...
		const memoryLimits = Runtime.resolveMemoryMapLimits({
		...
		Runtime.applyUfpsScaled(cartLayer.index.manifest.machine);
		const cpuHz = Runtime.resolveCpuHz(cartLayer.index.manifest.machine);
		const cycleBudgetPerFrame = calcCyclesPerFrameScaled(cpuHz, $.ufps_scaled);
		const memory = new Memory({
			engineRom: new Uint8Array(engineLayer.payload),
		...
		runtime.setTransferRatesFromManifest(cartLayer.index.manifest.machine);
	...
	private static resolveMemoryMapLimits(params: {
		...
		const limits = machineConfig.limits;
		...
		const engineAtlasSlotBytes = engineMachine.limits?.engine_atlas_slot_bytes;
		if (engineAtlasSlotBytes === undefined) {
			throw new Error('[Runtime] machine.limits.engine_atlas_slot_bytes is required in the engine manifest.');
		}
		if (!Number.isSafeInteger(engineAtlasSlotBytes) || engineAtlasSlotBytes <= 0) {
			throw new Error('[Runtime] machine.limits.engine_atlas_slot_bytes must be a positive integer.');
		}
		...
		if (limits?.asset_table_bytes !== undefined && assetTableBytes !== requiredAssetTableBytes) {
			throw new Error(`[Runtime] machine.limits.asset_table_bytes (${assetTableBytes}) must match required size ${requiredAssetTableBytes}.`);
		}
		const assetDataBaseOffset = IO_REGION_SIZE
			+ (stringHandleCount * STRING_HANDLE_ENTRY_SIZE)
			+ stringHeapBytes
			+ assetTableBytes;
		const skyboxFaceSize = machineConfig.skybox_face_size ?? SKYBOX_FACE_DEFAULT_SIZE;
		...
		if (limits?.asset_data_bytes !== undefined && assetDataBytes !== requiredAssetDataBytes) {
			throw new Error(`[Runtime] machine.limits.asset_data_bytes (${assetDataBytes}) must match required size ${requiredAssetDataBytes}.`);
		}
		...
		if (limits?.ram_bytes !== undefined && ramBytes !== computedRamBytes) {
			throw new Error(`[Runtime] machine.limits.ram_bytes (${ramBytes}) must match required size ${computedRamBytes}.`);
		}
	...
	private applyCartAssetLayers(): void {
		...
		Runtime.applyUfpsScaled($.assets.manifest.machine);
		const cpuHz = Runtime.resolveCpuHz($.assets.manifest.machine);
		this.setCpuHz(cpuHz);
		this.setCycleBudgetPerFrame(calcCyclesPerFrameScaled(cpuHz, $.ufps_scaled));
		this.setTransferRatesFromManifest($.assets.manifest.machine);
	}
	...
	private seedGlobals(): void {
		...
		const callClosureValue = (callee: Value, args: Value[], out: Value[]): void => {
			...
			const results = this.callClosure(callee as Closure, args);
			out.length = 0;
			for (let index = 0; index < results.length; index += 1) {
				out.push(results[index]);
			}
		};
		const key = (name: string): StringValue => this.internString(name);
		...
		setKey(mathTable, 'type', createNativeFunction('math.type', (args, out) => {
			...
			if (Number.isInteger(value)) {
				out.push(this.internString('integer'));
				return;
			}
			out.push(this.internString('float'));
		}));
		...
		this.registerGlobal('poke', createNativeFunction('poke', (args, out) => {
			const address = args[0] as number;
			this.memory.writeValue(address, args[1]);
			out.length = 0;
		}));
		this.registerGlobal('type', createNativeFunction('type', (args, out) => {
			const value = args.length > 0 ? args[0] : null;
			out.push(this.typeOfValue(value));
		}));
		this.registerGlobal('tostring', createNativeFunction('tostring', (args, out) => {
			const value = args.length > 0 ? args[0] : null;
			out.push(this.valueToStringValue(value));
		}));
		this.registerGlobal('tonumber', createNativeFunction('tonumber', (args, out) => {
			if (args.length === 0) {
				out.push(null);
				return;
			}
		...
		this.registerGlobal('assert', createNativeFunction('assert', (args, out) => {
			const condition = args.length > 0 ? args[0] : null;
			if (!isTruthy(condition)) {
				const message = args.length > 1 ? this.valueToString(args[1]) : 'assertion failed!';
		...
		this.registerGlobal('error', createNativeFunction('error', (args, out) => {
			void out;
			const message = args.length > 0 ? this.valueToString(args[0]) : 'error';
			throw this.createApiRuntimeError(message);
		}));
		this.registerGlobal('setmetatable', createNativeFunction('setmetatable', (args, out) => {
			const target = args[0] as Table;
			const metatable = args.length > 1 ? (args[1] as Table) : null;
			target.setMetatable(metatable);
			out.push(target);
		}));
		this.registerGlobal('getmetatable', createNativeFunction('getmetatable', (args, out) => {
			const target = args[0] as Table;
			out.push(target.getMetatable());
		}));
		this.registerGlobal('rawequal', createNativeFunction('rawequal', (args, out) => {
			out.push(args[0] === args[1]);
		}));
		this.registerGlobal('rawget', createNativeFunction('rawget', (args, out) => {
			const target = args[0] as Table;
			const key = args.length > 1 ? args[1] : null;
			out.push(target.get(key));
		}));
		this.registerGlobal('rawset', createNativeFunction('rawset', (args, out) => {
			const target = args[0] as Table;
			const key = args[1];
		...
		this.registerGlobal('select', createNativeFunction('select', (args, out) => {
			const index = args[0];
		...
		this.registerGlobal('pcall', createNativeFunction('pcall', (args, out) => {
			const fn = args[0];
			...
			try {
				...
				out.length = 0;
				out.push(false, this.internString(extractErrorMessage(error)));
			}
		}));
		this.registerGlobal('xpcall', createNativeFunction('xpcall', (args, out) => {
			const fn = args[0];
			...
			try {
				...
				const handlerArgs: Value[] = [this.internString(extractErrorMessage(error))];
				callClosureValue(handler, handlerArgs, out);
				out.unshift(false);
			}
		}));
		this.registerGlobal('require', createNativeFunction('require', (args, out) => {
			const moduleName = this.requireString(args[0]).trim();
			out.push(this.requireModule(moduleName));
		}));
		this.registerGlobal('array', createNativeFunction('array', (args, out) => {
		...
		this.registerGlobal('print', createNativeFunction('print', (args, out) => {
			const parts: string[] = [];
			for (let index = 0; index < args.length; index += 1) {
				parts.push(this.formatValue(args[index]));
			}
		...
		setKey(stringTable, 'rep', createNativeFunction('string.rep', (args, out) => {
			...
			out.push(this.internString(output));
		}));
		setKey(stringTable, 'sub', createNativeFunction('string.sub', (args, out) => {
			...
			out.push(this.internString(text.slice(startUnit, endUnit)));
		}));
		setKey(stringTable, 'find', createNativeFunction('string.find', (args, out) => {
			...
			if (match.length > 1) {
				...
				for (let index = 1; index < match.length; index += 1) {
					...
					out.push(value === undefined ? null : this.internString(value));
				}
				return;
			}
			out.push(first, last);
		...
		setKey(stringTable, 'match', createNativeFunction('string.match', (args, out) => {
			...
			out.push(this.internString(match[0]));
		}));
		setKey(stringTable, 'gsub', createNativeFunction('string.gsub', (args, out) => {
			const source = this.requireString(args[0]);
			const pattern = args.length > 1 ? this.requireString(args[1]) : '';
			const replacement = args.length > 2 ? args[2] : this.internString('');
			...
			try {
				const renderReplacement = (match: RegExpExecArray): string => {
					...
					if (replacement instanceof Table) {
						...
						const keyValue = match.length > 1
							? this.internString(match[1])
							: this.internString(match[0]);
						const mapped = replacement.get(keyValue);
						return mapped === null ? match[0] : this.valueToString(mapped);
					}
					if (isNativeFunction(replacement) || (replacement !== null && typeof replacement === 'object' && 'protoIndex' in replacement)) {
						...
						if (match.length > 1) {
							for (let index = 1; index < match.length; index += 1) {
								const value = match[index];
								fnArgs.push(value === undefined ? null : this.internString(value));
							}
							if (fnArgs.length === 0) {
								fnArgs.push(this.internString(match[0]));
							}
						} else {
							fnArgs.push(this.internString(match[0]));
						}
				...
				out.push(this.internString(result), count);
			} finally {
		...
		setKey(stringTable, 'gmatch', createNativeFunction('string.gmatch', (args, out) => {
			...
			const iterator = createNativeFunction('string.gmatch.iterator', (_args, iterOut) => {
				...
				if (match.length > 1) {
					for (let index = 1; index < match.length; index += 1) {
						const value = match[index];
						iterOut.push(value === undefined ? null : this.internString(value));
					}
					return;
				}
				iterOut.push(this.internString(match[0]));
			});
			out.push(iterator);
		}));
		...
		setKey(stringTable, 'char', createNativeFunction('string.char', (args, out) => {
			...
			out.push(this.internString(result));
		}));
		setKey(stringTable, 'format', createNativeFunction('string.format', (args, out) => {
			const template = this.requireString(args[0]);
			const formatted = this.formatLuaString(template, args, 1);
			out.push(this.internString(formatted));
		}));
		this.registerGlobal('string', stringTable);
		...
		setKey(tableLibrary, 'concat', createNativeFunction('table.concat', (args, out) => {
			const target = args[0] as Table;
			const separator = args.length > 1 ? this.requireString(args[1]) : '';
			const length = target.length();
			...
			try {
				for (let index = startIndex; index <= endIndex; index += 1) {
					...
					parts.push(value === null ? '' : this.valueToString(value));
				}
				out.push(this.internString(parts.join(separator)));
			} finally {
		...
		this.registerGlobal('table', tableLibrary);
		...
		setKey(osTable, 'date', createNativeFunction('os.date', (args, out) => {
			const format = args.length > 0 && args[0] !== null ? this.requireString(args[0]) : '%c';
			...
			out.push(this.internString(formatOsDate(format, date)));
		}));
		this.registerGlobal('os', osTable);
		...
		const ipairsIterator = createNativeFunction('ipairs.iterator', (args, out) => {
			...
			if (isNativeObject(target)) {
				...
				if (Array.isArray(raw)) {
					...
					out.push(nextIndex, this.toRuntimeValue(value));
					return;
				}
		...
		this.registerGlobal('next', nextFn);
		this.registerGlobal('pairs', createNativeFunction('pairs', (args, out) => {
			const target = args[0];
			if (!(target instanceof Table) && !isNativeObject(target)) {
				const stack = this.buildLuaStackFrames()
					.map(frame => `${frame.source ?? '<unknown>'}:${frame.line ?? '?'}:${frame.column ?? '?'}`)
					.join(' <- ');
				throw this.createApiRuntimeError(`pairs expects a table or native object (got ${this.formatValue(target)}). stack=${stack}`);
			}
			out.push(nextFn, target, null);
		}));
		this.registerGlobal('ipairs', createNativeFunction('ipairs', (args, out) => {
			const target = args[0];
			if (!(target instanceof Table) && !isNativeObject(target)) {
				throw this.createApiRuntimeError('ipairs expects a table or native object.');
			}
		...
		for (const { name, kind, descriptor } of members) {
			if (kind === 'method') {
				...
				this.registerGlobal(name, native);
				continue;
			}
			if (descriptor.get) {
	...
	private typeOfValue(value: Value): StringValue {
		if (value === null) {
			return this.internString('nil');
		}
		if (typeof value === 'boolean') {
			return this.internString('boolean');
		}
		if (typeof value === 'number') {
			return this.internString('number');
		}
		if (isStringValue(value)) {
			return this.internString('string');
		}
		if (value instanceof Table) {
			return this.internString('table');
		}
		if (isNativeFunction(value)) {
			return this.internString('function');
		}
		if (isNativeObject(value)) {
			return this.internString('native');
		}
		return this.internString('function');
	}
	...
	private exposeObjects(): void {
		...
		for (const [name, object] of entries) {
			this.registerGlobal(name, this.getOrCreateNativeObject(object));
		}
	...
	public async reloadProgramAndResetWorld(options?: { runInit?: boolean; }): Promise<void> {
		...
		try {
			...
			Runtime.applyUfpsScaled(manifest.machine);
			const cpuHz = Runtime.resolveCpuHz(manifest.machine);
			this.setCpuHz(cpuHz);
			this.setCycleBudgetPerFrame(calcCyclesPerFrameScaled(cpuHz, $.ufps_scaled));
			this.setTransferRatesFromManifest(manifest.machine);
		}
		finally {
			this.luaGate.end(gateToken);
		}
