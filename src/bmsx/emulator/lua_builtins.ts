...
export const DEFAULT_LUA_BUILTIN_FUNCTIONS: ReadonlyArray<LuaBuiltinDescriptor> = [
	...
	{ name: 'resolve_sys_rom_asset_range', params: ['asset_id'], signature: 'resolve_sys_rom_asset_range(asset_id)', description: 'Returns the ROM base and byte range for a system-visible asset id (overlay/cart first, then system).' },
	{ name: 'resolve_rom_asset_range', params: ['asset_id'], signature: 'resolve_rom_asset_range(asset_id)', description: 'Returns the ROM base and byte range for a system-visible asset id.' },
	...
	{ name: 'memwrite', params: ['base', '...'], signature: 'memwrite(base, ...)', description: 'Reserved compiler intrinsic for burst word writes. Lowers directly to a memory-write opcode and is not a first-class Lua value.' },
		...
		{ name: 'memf32le', params: ['addr'], signature: 'memf32le[addr]', description: 'Reserved memory-mapped little-endian 32-bit float space for direct `memf32le[addr]` reads/writes. Not a first-class Lua value. Writes to read-only locations are ignored.' },
		{ name: 'memf64le', params: ['addr'], signature: 'memf64le[addr]', description: 'Reserved memory-mapped little-endian 64-bit float space for direct `memf64le[addr]` reads/writes. Not a first-class Lua value. Writes to read-only locations are ignored.' },
	...
	{ name: 'sys_vdp_status', params: [], signature: 'sys_vdp_status', description: 'VDP status register; read submit/vblank bits.' },
