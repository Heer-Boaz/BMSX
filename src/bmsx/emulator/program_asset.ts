...
export type ProgramConstRelocKind = 'bx' | 'rk_b' | 'rk_c' | 'gl' | 'sys';
...
function decodeProgramLink(value: unknown): ProgramLink {
	...
	for (let index = 0; index < relocValues.length; index += 1) {
		...
		if (kind !== 'bx' && kind !== 'rk_b' && kind !== 'rk_c' && kind !== 'gl' && kind !== 'sys') {
			throw new Error(`ProgramAsset.link.constRelocs[${index}].kind must be 'bx', 'rk_b', 'rk_c', 'gl', or 'sys'.`);
		}
		constRelocs[index] = {
