...
import type { BmsxConsoleMetadata, ConsoleViewport, ConsoleResourceDescriptor, ConsoleLuaHoverRequest, ConsoleLuaHoverResult, ConsoleLuaHoverScope, ConsoleLuaHoverValueState, ConsoleLuaResourceCreationRequest, ConsoleLuaDefinitionLocation, ConsoleLuaSymbolEntry, ConsoleLuaBuiltinDescriptor } from './types';
...
type TopBarButtonId = 'resume' | 'reboot' | 'save' | 'resources' | 'filter' | 'resolution' | 'wrap';
...
const COLOR_FRAME = 15; // White
const COLOR_TOP_BAR = 14; // Grey
const COLOR_TOP_BAR_TEXT = 1; // Black
const COLOR_CODE_BACKGROUND = 4; // Dark blue
const COLOR_GUTTER_BACKGROUND = 14; // Grey
const COLOR_CODE_TEXT = 15; // Grey
...
const COLOR_STATUS_TEXT = 1; // Black
const COLOR_STATUS_WARNING = 9;
const COLOR_STATUS_SUCCESS = 10;
const COLOR_STATUS_ERROR = 2;
const COLOR_STATUS_ALERT = 6;
const COLOR_RESOURCE_PANEL_BACKGROUND = 14; // Grey
const COLOR_RESOURCE_PANEL_TEXT = 0;
const COLOR_RESOURCE_PANEL_HIGHLIGHT = 0;
const COLOR_RESOURCE_PANEL_HIGHLIGHT_TEXT = 15; // White
const COLOR_RESOURCE_VIEWER_BACKGROUND = 0;
const COLOR_RESOURCE_VIEWER_TEXT = 15; // White
const COLOR_SEARCH_BACKGROUND = 7; // Light green
const COLOR_SEARCH_TEXT = 1; // Black
const COLOR_SEARCH_PLACEHOLDER = COLOR_CODE_DIM;
const COLOR_SEARCH_OUTLINE = 1; // Black
...
const CREATE_RESOURCE_MAX_PATH_LENGTH = 1024;
...
export class ConsoleCartEditor {
	...
	private handleEditorInput(keyboard: KeyboardInput, deltaSeconds: number): void {
		...
		if ((ctrlDown || metaDown) && shiftDown && this.isKeyJustPressed(keyboard, 'KeyL')) {
...
private drawRuntimeErrorOverlay(api: BmsxConsoleApi, codeTop: number, codeRight: number, textLeft: number): void {
	...
	private toggleResourcePanelFilterMode(): void {
		...
		this.showMessage(`Files panel: showing ${modeLabel}`, COLOR_STATUS_TEXT, 2.5);
	}
...
private drawParameterHintOverlay(api: BmsxConsoleApi, bounds: { codeTop: number; codeBottom: number; codeLeft: number; codeRight: number; textLeft: number }): void {
	...
	private resolveParameterHintContext(): ParameterHintState | null {
		...
		if (objectName && objectName.toLowerCase() === 'api') {
			const apiMeta = ConsoleCartEditor.API_COMPLETION_DATA.signatures.get(methodName);
			if (apiMeta) {
