...
import { api, BmsxConsoleRuntime, CONSOLE_BUTTON_ACTIONS } from './runtime';
...
export function registerApiBuiltins(interpreter: LuaInterpreter): void {
	...
	const resolveButtonAction = (value: LuaValue, fnName: string): string => {
		...
		if (index < 0 || index >= CONSOLE_BUTTON_ACTIONS.length) {
			throw runtime.createApiRuntimeError(`${fnName}(button [, player]) button index must be between 0 and ${CONSOLE_BUTTON_ACTIONS.length - 1}.`);
		}
...
export function registerLuaBuiltin(metadata: ConsoleLuaBuiltinDescriptor): void {
	const runtime = BmsxConsoleRuntime.instance;
