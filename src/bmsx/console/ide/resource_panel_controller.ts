...
export class ResourcePanelController {
  ...
  // === Keyboard ===
  handleKeyboard(keyboard: KeyboardInput, _deltaSeconds: number): void {
    if (!this.visible) return;
    const ctrlDown = isModifierPressedGlobal(this.host.playerIndex, 'ControlLeft') || isModifierPressedGlobal(this.host.playerIndex, 'ControlRight');
    const metaDown = isModifierPressedGlobal(this.host.playerIndex, 'MetaLeft') || isModifierPressedGlobal(this.host.playerIndex, 'MetaRight');
    const shiftDown = isModifierPressedGlobal(this.host.playerIndex, 'ShiftLeft') || isModifierPressedGlobal(this.host.playerIndex, 'ShiftRight');
    if ((ctrlDown || metaDown) && shiftDown && isKeyJustPressedGlobal(this.host.playerIndex, 'KeyR')) {
    ...
    if ((ctrlDown || metaDown) && isKeyJustPressedGlobal(this.host.playerIndex, 'KeyB')) {
      consumeKeyboardKey(keyboard, 'KeyB');
      this.togglePanel();
      return;
    }
    if (isKeyJustPressedGlobal(this.host.playerIndex, 'Escape')) {
      consumeKeyboardKey(keyboard, 'Escape');
      this.hide();
      return;
    }
    if (isKeyJustPressedGlobal(this.host.playerIndex, 'Tab')) {
    ...
    const horizontalMoves: Array<{ key: string; predicate: boolean; delta: number }> = [
      { key: 'ArrowLeft', predicate: isKeyJustPressedGlobal(this.host.playerIndex, 'ArrowLeft'), delta: -horizontalStep },
      { key: 'ArrowRight', predicate: isKeyJustPressedGlobal(this.host.playerIndex, 'ArrowRight'), delta: horizontalStep },
    ];
    ...
    if (isKeyJustPressedGlobal(this.host.playerIndex, 'Enter')) {
    ...
    for (const entry of moves) {
      const triggered = isKeyJustPressedGlobal(this.host.playerIndex, entry.code);
      if (triggered) {
