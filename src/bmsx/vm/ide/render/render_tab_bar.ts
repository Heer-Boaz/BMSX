...
export function renderTabBar(api: BmsxVMApi, host: TabBarHost): number {
	...
	if (metrics.length === 0) {
		...
		api.rectfill(0, barTop, host.viewportWidth, barBottom, undefined, constants.COLOR_TAB_BAR_BACKGROUND);
		api.rectfill(0, Math.max(barTop, barBottom - 1), host.viewportWidth, barBottom, undefined, constants.COLOR_TAB_BORDER);
		return 1;
	}
	...
	for (const entry of layout) {
		...
		api.rect(bounds.left, bounds.top, bounds.right, bounds.bottom, undefined, constants.COLOR_TAB_BORDER);
		...
		if (entry.closable) {
			...
			if (hovered) {
				...
				if (entry.dirty && entry.markerMetrics) {
					...
					api.rectfill(markerX, markerY, markerRight, markerBottom, undefined, constants.COLOR_TAB_DIRTY_MARKER);
				}
			}
		} else {
			host.tabCloseButtonBounds.delete(tab.id);
		...
		if (active) {
			api.rectfill(bounds.left, bounds.bottom - 1, bounds.right, bounds.bottom, undefined, fillColor);
		}
	}
