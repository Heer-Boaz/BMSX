...
export class CompletionController {
	...
	private builtinDescriptors: VMLuaBuiltinDescriptor[] = null;
	private readonly builtinDescriptorMap: Map<string, VMLuaBuiltinDescriptor> = new Map();
	...
	private buildSymbolCompletionItems(entries: VMLuaSymbolEntry[], scope: 'local' | 'global' | 'module'): LuaCompletionItem[] {
	...
	private convertSourceRange(range: LuaSourceRange): VMLuaDefinitionRange {
		return {
	...
	private isPositionWithinRange(row: number, column: number, range: VMLuaDefinitionRange): boolean {
	...
	private isDefinitionBeforePosition(range: VMLuaDefinitionRange, row: number, column: number): boolean {
	...
	private definitionOccursAfter(candidate: VMLuaDefinitionRange, other: VMLuaDefinitionRange): boolean {
	...
	private formatSymbolKind(kind: VMLuaSymbolEntry['kind']): string {
		switch (kind) {
	...
	private ensureBuiltinDescriptorCache(force = false): void {
		...
		let descriptors: VMLuaBuiltinDescriptor[];
		...
		this.sharedCompletionItems = null;
		this.sharedCompletionMap = null;
		const registerDescriptor = (descriptor: VMLuaBuiltinDescriptor): void => {
			...
			const entry: VMLuaBuiltinDescriptor = {
				name: normalized,
				params,
				signature,
				optionalParams,
	...
	private findBuiltinDescriptor(objectName: string, methodName: string): VMLuaBuiltinDescriptor {
		this.ensureBuiltinDescriptorCache();
