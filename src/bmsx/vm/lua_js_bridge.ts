...
export class LuaJsBridge implements LuaInteropAdapter {
	...
	constructor(vmRuntime: BmsxVMRuntime, luaHandlerCache: LuaHandlerCache) {
	...
	public wrapNativeValue(value: object | Function): LuaNativeValue {
		return this.vmRuntime.interpreter.getOrCreateNativeValue(value, resolveNativeTypeName(value));
	}
	...
	public parseSnapshotObjectId(text: string): number {
		...
		if (!Number.isFinite(id)) {
			throw new Error(`[BmsxVMRuntime] Invalid Lua snapshot object id '${text}'.`);
		}
		return id;
	}
	...
	public parseSnapshotReferenceId(raw: unknown): number {
		const id = Number(raw);
		if (!Number.isFinite(id)) {
			throw new Error(`[BmsxVMRuntime] Invalid Lua snapshot reference id '${String(raw)}'.`);
		}
		return id;
	}
	...
	public buildLuaTableSnapshotPayload(table: LuaTable, ctx: LuaSnapshotContext): unknown {
		...
		for (const [key, entryValue] of entries) {
			...
			try {
				...
				if ($.debug) {
					console.warn(`[BmsxVMRuntime] Skipping Lua table entry '${String(key)}' during snapshot:`, error);
				}
				continue;
			}
			...
			try {
				...
				if ($.debug) {
					console.warn(`[BmsxVMRuntime] Skipping Lua table key '${String(key)}' during snapshot:`, error);
				}
				continue;
			}
	...
	public wrapFunctionsInValue(
		...
		if (isLuaFunctionValue(value)) {
			...
			return this.vmRuntime.luaFunctionRedirectCache.getOrCreate(moduleId, path, value);
		}
		if (!isLuaTable(value)) {
			return value;
		}
