...
export class BmsxVMStorage {
	...
	public setValue(index: number, value: number): void {
		...
		if (!Number.isFinite(value)) {
			throw new Error(`[BmsxVMStorage] Attempted to persist non-finite value '${value}' at index ${index}.`);
		}
	...
	public getValue(index: number): number {
		...
		if (!Number.isFinite(parsed)) {
			throw new Error(`[BmsxVMStorage] Stored value '${raw}' at index ${index} is not a finite number.`);
		}
		this.touchedIndices.add(index);
		return parsed;
	}
	...
	public restore(state: { namespace: string; entries: Array<{ index: number; value: number }> }): void {
		...
		for (const entry of state.entries) {
			...
			if (!Number.isFinite(entry.value)) {
				throw new Error(`[BmsxVMStorage] Restored value '${entry.value}' at index ${entry.index} is not finite.`);
			}
	...
	private validateNamespace(ns: string): string {
		if (!ns || ns.trim().length === 0) {
			throw new Error('[BmsxVMStorage] Namespace must be a non-empty string.');
		}
		return ns.trim();
	}
	...
	private assertIndex(index: number): void {
		if (!Number.isInteger(index) || index < 0) {
			throw new Error(`[BmsxVMStorage] Index '${index}' must be a non-negative integer.`);
		}
	}
}
