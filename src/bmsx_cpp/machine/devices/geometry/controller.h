...
private:
	...
	bool readOverlapInstanceAt(const GeoJob& job, uint32_t instanceIndex, std::array<uint32_t, GEO_OVERLAP2D_INSTANCE_WORDS>& out) const;
	bool processOverlap2dPair(GeoJob& job, uint32_t recordIndex, const std::array<uint32_t, GEO_OVERLAP2D_INSTANCE_WORDS>& instanceA, const std::array<uint32_t, GEO_OVERLAP2D_INSTANCE_WORDS>& instanceB, uint32_t pairMeta);
	...
	void projectVertexSpanInto(uint32_t base, uint32_t count, double ax, double ay, ProjectionScratch& out) const;
	...
	std::optional<uint32_t> resolveByteOffset(uint32_t base, uint64_t offset, uint64_t byteLength) const;
	...
	std::optional<uint32_t> resolveIndexedSpan(uint32_t base, uint32_t index, uint32_t stride, uint64_t byteLength) const;
	...
	std::array<uint32_t, GEO_OVERLAP2D_INSTANCE_WORDS> m_overlapInstanceA = { 0, 0, 0, 0, 0 };
	std::array<uint32_t, GEO_OVERLAP2D_INSTANCE_WORDS> m_overlapInstanceB = { 0, 0, 0, 0, 0 };
