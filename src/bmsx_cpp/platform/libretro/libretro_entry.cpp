...
void retro_init(void) {
    ...
    if (!environ_cb(RETRO_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt)) {
        logging.log(RETRO_LOG_WARN, "[BMSX] XRGB8888 not supported, trying RGB565\n");
...
void retro_get_system_av_info(struct retro_system_av_info* info) {
    ...
    constexpr unsigned BASE_WIDTH = 256;
    constexpr unsigned BASE_HEIGHT = 224;
    ...
    info->geometry.aspect_ratio = static_cast<float>(BASE_WIDTH) / static_cast<float>(BASE_HEIGHT);
...
bool retro_load_game(const struct retro_game_info* game) {
    if (!game) {
        logging.log(RETRO_LOG_INFO, "[BMSX] No game provided, loading empty cart\n");
    ...
    if (game->data && game->size > 0) {
        return g_platform->loadRom(static_cast<const uint8_t*>(game->data), game->size);
    } else if (game->path) {
...
size_t retro_serialize_size(void) {
    if (g_platform) {
        return g_platform->getStateSize();
    }
    return 0;
}
