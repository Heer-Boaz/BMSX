...
void commitVdpSkyboxViewState(GameView& view, const VDP& vdp) {
	...
	const SkyboxFaceSources& sources = vdp.committedSkyboxFaceSources();
	const std::array<const VdpSlotSource*, SKYBOX_FACE_COUNT> faces = {{&sources.posx, &sources.negx, &sources.posy, &sources.negy, &sources.posz, &sources.negz}};
	for (size_t index = 0; index < SKYBOX_FACE_COUNT; ++index) {
		const VDP::BlitterSource source = vdp.resolveBlitterSource(*faces[index]);
