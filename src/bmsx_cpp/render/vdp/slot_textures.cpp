...
void VdpSlotTextures::consumeVdpSurfaceUpload(const VdpSurfaceUpload& upload) {
	...
	if (forceFullUpload) {
		const TextureHandle handle = m_textureManager.resizeTextureForKey(surface.textureKey, static_cast<i32>(width), static_cast<i32>(height), RAW_RGBA_TEXTURE_PARAMS);
...
auto VdpSlotTextures::readSurfaceTextureHandle(u32 surfaceId) const -> TextureHandle {
	return m_textureManager.getTextureByUri(resolveVdpSurfaceTextureKey(surfaceId), RAW_RGBA_TEXTURE_PARAMS);
}
...
void VdpSlotTextures::uploadVdpSlotRows(const std::string& textureKey, const VdpSurfaceUpload& upload, u32 rowStart, u32 rowEnd) {
	...
	m_view.backend()->updateTextureRegion(
		m_textureManager.getTextureByUri(textureKey, RAW_RGBA_TEXTURE_PARAMS),
		pixels,
...
void VdpSlotTextures::initializeVdpSlotTexture(const VdpSurfaceUpload& upload) {
	const VdpRenderSurfaceInfo surface = resolveVdpRenderSurfaceForUpload(upload);
	m_textureManager.createTextureFromPixelsSync(surface.textureKey, EMPTY_TEXTURE_SEED.data(), 1, 1, RAW_RGBA_TEXTURE_PARAMS);
