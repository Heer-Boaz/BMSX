...
constexpr i32 MESH_SURFACE_BLEND = 2;

struct MeshStreamVertex {
	f32 x = 0.0f;
	f32 y = 0.0f;
	f32 z = 0.0f;
	f32 nx = 0.0f;
...
struct MeshDrawStream {
	const MeshStreamVertex* vertices = nullptr;
...
private:
	std::array<MeshStreamVertex, VDP_MDU_VERTEX_LIMIT> m_vertices{};
