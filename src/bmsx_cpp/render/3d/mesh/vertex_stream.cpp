...
void writeMeshVertex(MeshGLES2Vertex& target,
	...
	if (skinningEnabled) {
		Vec3 skinned{};
		writeSkinnedPositionInto(skinned, jointMatrices, mesh, vertexIndex, x, y, z, jointCount);
		x = skinned.x;
		y = skinned.y;
		z = skinned.z;
	}
...
MeshGLES2DrawStream MeshVertexStreamBuilder::build(const GameView& view,
	...
	if (mesh.indices.empty()) {
		for (size_t index = 0u; index < outputVertexCount; ++index) {
			writeMeshVertex(m_vertices[index], m_jointMatrices, m_morphWeights, mesh, index, morphCount, skinningEnabled, entry.jointCount, baseColor);
		}
	} else {
		for (size_t index = 0u; index < outputVertexCount; ++index) {
			writeMeshVertex(m_vertices[index], m_jointMatrices, m_morphWeights, mesh, mesh.indices[index], morphCount, skinningEnabled, entry.jointCount, baseColor);
		}
	}
	return {m_vertices.data(), m_vertexCount, &m_modelMatrix};
}
