...
RunResult Runtime::runWithBudget() {
	...
	// if (!m_debugRunReportInitialized) {
	// 	m_debugRunReportInitialized = true;
	// 	m_debugRunReportAt = now;
	// }
	// m_debugRunCount += 1;
	// m_debugRunCountTotal += 1;
	...
	// 	m_debugRunYields += 1;
	// 	m_debugRunYieldsTotal += 1;
	// }
	// m_debugRunRemainingAcc += remaining;
	// const double elapsedMs = to_ms(now - m_debugRunReportAt);
	// if (elapsedMs >= 1000.0) {
	// 	m_debugRunReportAt = now;
	// 	m_debugRunCount = 0;
	// 	m_debugRunYields = 0;
	// 	m_debugRunRemainingAcc = 0.0;
	// }
	return result;
}
...
void Runtime::tickUpdate() {
	if (!m_luaInitialized || !m_tickEnabled || m_runtimeFailed) {
		...
		// 	std::cerr << "[Runtime] update: skipped (initialized=" << (m_luaInitialized ? "true" : "false")
...
void Runtime::tickDraw() {
	if (!m_luaInitialized || !m_tickEnabled || m_runtimeFailed) {
		...
		// 	std::cerr << "[Runtime] draw: skipped (initialized=" << (m_luaInitialized ? "true" : "false")
		// 			  << " tick=" << (m_tickEnabled ? "true" : "false")
		// 			  << " failed=" << (m_runtimeFailed ? "true" : "false") << ")" << std::endl;
		// 	--s_drawLogRemaining;
		// }
	...
	// const i64 yieldsThisFrame = m_debugRunYieldsTotal - m_debugTickYieldsBefore;
	// m_debugFrameCount += 1;
	// m_debugFrameCyclesUsedAcc += cyclesUsed;
	// m_debugFrameRemainingAcc += static_cast<double>(m_frameState.cycleBudgetRemaining);
	// m_debugFrameYieldsAcc += static_cast<double>(yieldsThisFrame);
	// m_debugFrameGrantedAcc += static_cast<double>(m_frameState.cycleBudgetGranted);
	// m_debugFrameCarryAcc += static_cast<double>(m_frameState.cycleCarryGranted);
	// const double frameElapsedMs = to_ms(frameNow - m_debugFrameReportAt);
	// if (frameElapsedMs >= 1000.0) {
	// 	m_debugFrameReportAt = frameNow;
	// 	m_debugFrameCount = 0;
	// 	m_debugFrameCyclesUsedAcc = 0.0;
	// 	m_debugFrameRemainingAcc = 0.0;
	// 	m_debugFrameYieldsAcc = 0.0;
	// 	m_debugFrameGrantedAcc = 0.0;
	// 	m_debugFrameCarryAcc = 0.0;
	// }
...
void Runtime::executeUpdateCallback(double deltaSeconds) {
	...
	try {
		...
		// 	std::fprintf(stderr,
		// 		"[Runtime] update perf avg total=%.2f run=%.2f engine=%.2f io=%.2f max_total=%.2f max_run=%.2f max_engine=%.2f max_io=%.2f frames=%llu\n",
		...
		// }
		// if (s_updateLogRemaining > 0) {
		// 	const char* pendingLabel = m_pendingCall == PendingCall::None
		// 		? "none"
		// 		: (m_pendingCall == PendingCall::Update ? "update" : "draw");
		// std::cerr << "[Runtime] update: run=" << (m_updateFn ? "yes" : "no")
...
void Runtime::executeDrawCallback() {
	...
	try {
		...
		// 	const double invFrames = 1.0 / static_cast<double>(accFrames);
		// 	std::fprintf(stderr,
		// 		"[Runtime] draw perf avg total=%.2f run=%.2f engine=%.2f io=%.2f max_total=%.2f max_run=%.2f max_engine=%.2f max_io=%.2f frames=%llu\n",
		// 		accTotalMs * invFrames,
		// 		accRunMs * invFrames,
		// 		accEngineMs * invFrames,
		// 		accIoMs * invFrames,
		...
		// 		? "none"
		// 		: (m_pendingCall == PendingCall::Update ? "update" : "draw");
		// std::cerr << "[Runtime] draw: run=" << (m_drawFn ? "yes" : "no")
		// 	          << " pending=" << pendingLabel
