module Sintervania.Model.Projectiles {
	/*[Serializable]*/
	export class TriRoe extends PlayerProjectile {
		...
		// });
		public get hitarea(): IArea {
			if (!M._.Belmont.roeState.Roeing || M._.Belmont.RecoveringFromHit)
				return null;
			if (!M._.Belmont.Crouching)
				return TriRoe.hitareas[M._.Belmont.ImageId] + Belmont.RoeState.RoeSpritePosOffset[M._.Belmont.Direction][M._.Belmont.roeState.CurrentFrame];
			return TriRoe.hitareas[M._.Belmont.ImageId] + Belmont.RoeState.RoeSpritePosOffsetCrouching[M._.Belmont.Direction][M._.Belmont.roeState.CurrentFrame];
		}
		public set hitarea(value: IArea) {
		...
		constructor(pos: Point, dir: Direction) {
			super(new Point(pos.x, pos.y), new Point(0, 0));
			this.Direction = dir;
