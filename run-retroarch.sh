...
ROM_ARG="${1:-}"
if [[ -z "$ROM_ARG" ]]; then
	echo "Usage: $(basename "$0") <romfile>" >&2
	echo "Example: $(basename "$0") 2025.debug.rom" >&2
	echo "Build with: npm run build:libretro:debug <rompack-folder-name>" >&2
	exit 1
fi
case "$ROM_ARG" in
	*.rom | *.bmsx) ;;
...
gdb --batch -ex "set debuginfod enabled off" -ex "set pagination off" -ex "run" -ex "bt" --args "$RETROARCH_BIN" --appendconfig="$LOCAL_CFG" -L "$CORE_DST" "$ROM_PATH"
