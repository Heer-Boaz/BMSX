...
SNESMINI_CMAKE_ARGS ?= -DBMSX_BUILD_LIBRETRO=ON -DBMSX_ENABLE_GLES2=ON -DBMSX_ENABLE_ZLIB=OFF -DGLESV2_LIBRARY=$(SNESMINI_SYSROOT_USR_LIB_DIR)/libGLESv2.so -DCMAKE_C_FLAGS="$(SNESMINI_C_FLAGS)" -DCMAKE_CXX_FLAGS="$(SNESMINI_CXX_FLAGS)" -DCMAKE_CXX_STANDARD=17 -DCMAKE_SYSROOT="$(SNESMINI_SYSROOT)" -DCMAKE_EXE_LINKER_FLAGS="$(SNESMINI_LINK_FLAGS)" -DCMAKE_SHARED_LINKER_FLAGS="$(SNESMINI_LINK_FLAGS)"
