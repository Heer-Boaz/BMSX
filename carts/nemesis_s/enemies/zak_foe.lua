...
local update_phase_elapsed<const> = function(self, duration_ms)
	...
	self.phase_elapsed_ms = elapsed - duration_ms
	return true
end
...
function zak_foe:update_prepare_jump()
...
function zak_foe:update_jumping()
	...
	if update_phase_elapsed(self, zak_foe_jump_ms) then
...
function zak_foe:update_recovering()
	if self:dispose_if_left_of_stage(zak_foe_width) then
		return
	end
	update_weapon(self)
...
local define_fsm<const> = function()
	fsm_library.register(ids_zak_foe_fsm, {
		...
		states = {
			prepare_jump = {
				update = zak_foe.update_prepare_jump,
			},
			jumping = {
			...
			recovering = {
				...
				update = zak_foe.update_recovering,
			},
		},
	})
end
