...
function player:spawn_bullet(vessel_id, bullet)
	...
	for vessel_id = 1, vessel_count do
		...
		if vessel_y < 166 and vessel_x < 217 then
			if laser_level > 0 then
				...
				local secondary<const> = self.secondary_projectiles[vessel_id]
				if primary.type == projectile_type_none then
					self:spawn_bullet(vessel_id, primary)
				elseif secondary.type == projectile_type_none then
					self:spawn_bullet(vessel_id, secondary)
				elseif telemetry_enabled then
					self:emit_event(
						'weapon_blocked',
...
function player:update_missile(missile)
	local stage<const> = self.stage
	local row_tile_count<const> = stage.tile_columns
	if stage:first_solid_tile_offset(missile.x, missile.y + 8, row_tile_count) == row_tile_count then
		missile.sprite = weapon_sources.missile_falling
	...
	elseif stage:first_solid_tile_offset(
		(missile.x + 8) & 0xff,
		missile.y + 8,
		row_tile_count
	) == row_tile_count then
	...
	if stage:first_solid_tile_offset(missile.x, missile.y, row_tile_count) < row_tile_count then
		self:despawn_missile(missile, 'stage_collision')
	end
end
