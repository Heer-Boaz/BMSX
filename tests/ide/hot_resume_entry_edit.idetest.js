...
const initPrintBreakpointLine = entryRecord.base_src
...
t.assert(
	...
	'rejected edit reran init participant',
);
t.assert(
...
t.openLuaSource('entry.lua');
t.replaceActiveCodeSource(entryRecord.base_src.replace(
	'refresh_hot_resume_probe<init>',
	'refresh_hot_resume_probe',
));
await t.performHotResume();
await t.frames(60);

t.assert(sourceState.currentBlua32Media === originalMedia, 'participant removal replaced tooling media');
t.assert(cpu.getFrameDepth() === frameDepthBeforeRejectedEdit, 'participant removal changed frame depth');
t.assert(
	cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_init_count')) === initCountBeforeRejectedEdit,
	'participant removal executed guest code',
);
...
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_module_probe')) === 1, 'first module edit did not execute through init participant');
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_system_probe')) === 1, 'first system-ROM edit did not execute');
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_init_count')) === 2, 'first Hot Resume did not rerun init participant exactly once');
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_start_game_count')) === 1, 'first Hot Resume reran cold game creation');
...
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_module_probe')) === 2, 'second consecutive module edit did not execute through init participant');
...
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_init_count')) === 3, 'second Hot Resume did not rerun init participant exactly once');
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_start_game_count')) === 1, 'second Hot Resume reran cold game creation');
...
t.assert(
	...
	'system-only Hot Resume reran the mechanically relinked cartridge init participant',
);
...
t.assert(
	...
	'init participant did not reach the print breakpoint',
);
t.assert(
...
t.assert(!t.debuggerStopped(), 'debugger remained stopped after continuing the init participant');
t.assert(
	cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_init_count')) === initCountBeforeBreakpoint + 1,
	'continuing the init breakpoint reran the participant',
);
t.assert(
...
t.assert(cpu.getGlobalByKey(cpu.stringPool.intern('hot_resume_start_game_count')) === 1, 'no-op Hot Resume reran cold game creation');
...
t.assert(rebootedCpu.getGlobalByKey(rebootedCpu.stringPool.intern('hot_resume_init_count')) === 1, 'cold boot did not execute the init participant exactly once');
t.assert(rebootedCpu.getGlobalByKey(rebootedCpu.stringPool.intern('hot_resume_start_game_count')) === 1, 'cold boot did not execute game creation exactly once');
