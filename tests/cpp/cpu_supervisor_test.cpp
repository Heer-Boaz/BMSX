...
void testPhysicalFunctionRecordsRemainExecutionAuthority() {
	...
	bmsx::Blua32DecodedExecutionImage decoded =
		machine.executionAddressSpace.resolveSystemDomain();
	decoded.layout.functions[SYSTEM_IRQ_FUNCTION].codeAddress = 0u;
	machine.cpu.replaceExecutionImage(std::move(decoded));

	machine.memory.writeMappedU32LE(bmsx::IO_IRQ_MASK, bmsx::IRQ_VBLANK);
	machine.irq.raise(bmsx::IRQ_VBLANK);
	require(machine.cpu.enterPendingInterrupt(), "supervisor IRQ enters after media replacement");
	require(
		machine.cpu.readFramePc(1) == machine.systemRom.textAddress,
		"IRQ execution reads the physical function record instead of the mutated host decode"
	);
}

void testPhysicalUpvalueRecordsRemainExecutionAuthority() {
	bmsx::test::Blua32TestImage image;
	image.text.resize(9u * bmsx::INSTRUCTION_BYTES);
	image.constants = {static_cast<bmsx::f64>(7.0)};
	image.globalNames = {"physical_upvalue_seen"};
	std::span<bmsx::u8> code(image.text);
	...
	bmsx::writeInstruction(code, 4, static_cast<bmsx::u8>(bmsx::OpCode::SETGL), 1, 0, 0);
	bmsx::writeInstruction(code, 5, static_cast<bmsx::u8>(bmsx::OpCode::HALT), 0, 0, 0);
	bmsx::writeInstruction(code, 6, static_cast<bmsx::u8>(bmsx::OpCode::GETUP), 0, 0, 0);
	...
	image.functions = {
		{.firstWord = 0u, .wordCount = 6u, .maxStack = 2u},
		{
			...
			.wordCount = 2u,
			.staticClosure = false,
			.upvalues = {{.inStack = true, .index = 0u}},
		},
	...
	bmsx::Blua32DecodedExecutionImage decoded =
		machine.executionAddressSpace.resolveSystemDomain();
	decoded.layout.functions[1].upvalues[0] = {
		.inStack = false,
		.index = 31u,
	};
	machine.cpu.replaceExecutionImage(std::move(decoded));
	...
	require(
		...
		"physical upvalue fixture reaches HALT"
	);
	const bmsx::Value key =
		bmsx::valueString(machine.cpu.stringPool().intern("physical_upvalue_seen"));
	require(
		bmsx::asNumber(machine.cpu.getGlobalByKey(key)) == 7.0,
		"CLOSURE reads the physical upvalue table instead of the mutated host decode"
	);
}
