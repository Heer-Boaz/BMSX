...
return {kind = 'integration', tests = {
		projectile_datapaths = function(t)
		...
		held_fire_cadence = function(t)

			local _director<const>, _stage<const>, player<const> = fixture.start_game(t)
			local test<const> = {salvo_count = 0, actioneffect_recorder = actioneffect_recorder.new(player.actioneffects, 8)}
			t:observe_actioneffects(test.actioneffect_recorder)
			player.fire_weapon_salvo = function() test.salvo_count = test.salvo_count + 1 end
			t:down('Space')
			t:wait_until('fire input edge', function() return test.salvo_count > 0 end, 10)
			...
			local repeat_start_ms<const> = world.gameplay_time_ms
			...
			t:wait_until('held-fire repeat', function()
				local elapsed_ms<const> = world.gameplay_time_ms - repeat_start_ms
				if elapsed_ms < repeat_period_ms then
					assert(test.salvo_count == 1, 'held fire repeated before fifteen updates')
					return false
				end
				return test.salvo_count > 1 or elapsed_ms > repeat_deadline_ms
			end, 120)
			assert(test.salvo_count == 2, 'held fire did not repeat at the original E437 cadence')
			assert(world.gameplay_time_ms - repeat_start_ms <= repeat_deadline_ms,
			'held fire skipped its first repeat boundary')
			assert(test.actioneffect_recorder[4] == 2, 'periodic execution was misreported as a trigger attempt')
			t:up('Space')
			t:wait_until('fire release edge', function() return test.actioneffect_recorder[4] > 2 end, 10)
