...
return {
	...
	tests = {
		world1_daemon = function(t)
			...
			assert(daemon ~= nil, 'world daemon was not admitted')
			...
			t:wait_until('first daemon burst', function()
				assert(daemon.visible and daemon.collider.enabled and daemon.behaviour.enabled, 'daemon remained inactive after appearance')
				return #daemon.spawn_projectiles > 0
			end, 1000)
			...
			*apu_slot = 0
			...
			t:wait_until('daemon death unlocks key', function()
				assert(not daemon.behaviour.enabled and not daemon.collider.enabled, 'daemon combat survived defeat')
				assert(world.gameplay_clock_running, 'daemon death suspended all gameplay time')
				if registry:get('world1_daemon_key') ~= nil then return true end
				assert(player.state_machines:matches_state(test.player_defeated_state), 'player control lock ended before death')
				return false
			end, 1000)
			do
			...
			t:wait_until('HUD health drains', function() return registry:get('ui').hud_health_level ~= player.status.max_health end, 120)
			player.x = test.key_x
			player.y = test.key_y
			t:wait_until('key pickup', function() return player.status.inventory_items.keyworld1 end, 120)
			do
				local key<const> = registry:get('world1_daemon_key')
			...
			t:wait_until('victory dance disposes key', function() return registry:get('world1_daemon_key') == nil end, 1000)
			assert(director.state_machines:matches_state(test.director_game_completion_state),
