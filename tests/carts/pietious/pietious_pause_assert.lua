...
return {
	...
	tests = {
		pause = function(t)
			...
			local assert_paused<const> = function()
			...
			t:down('F2')
			t:wait_until('pause admission', function() return director.state_machines:matches_state(test.pause_state) and not world.gameplay_clock_running end, 30)
			local enemy<const> = registry:get('probe.pause.enemy')
			local projectile<const> = registry:get('probe.pause.projectile')
			...
			t:up('F2')
			t:wait_until('600-VBlank seated countdown', function()
				assert_paused()
				local wait<const> = player.timelines:get('p.tl.pw')
				if not wait.playing then return true end
			...
			t:wait_until('stuck animation cycle', function()
				assert_paused()
				local animation<const> = player.timelines:get('p.tl.pa')
				...
				return test.animation_ticks >= flow_pause_stuck_frame_hold * 2
			end, 120)
			do
				local head<const> = player.timelines:get('p.tl.pa').head
			...
			t:press('F2', 4)
			t:wait_until('pause resume', function() return director.state_machines:matches_state(test.room_state) and world.gameplay_clock_running end, 30)
			assert(player.state_machines:matches_state(test.quiet_state),
			...
			t:wait_until('gameplay clock resumes', function() return world.gameplay_time_ms > test.resume_gameplay_time_ms end, 12)

			t:wait_until('retained objects resume movement', function()
				local enemy<const> = registry:get('probe.pause.enemy')
				local projectile<const> = registry:get('probe.pause.projectile')
				return (enemy ~= nil and (enemy.x ~= test.enemy_x or enemy.y ~= test.enemy_y))
				or (projectile ~= nil and (projectile.x ~= test.projectile_x or projectile.y ~= test.projectile_y))
			end, 12)
			assert(world.gameplay_time_ms > test.resume_gameplay_time_ms, 'gameplay time did not resume')
