...
for (const functions of [32, 1024]) {
	...
	for (let index = 0; index < functions; index += 1) {
		declarations.push(`function api.create_${index}(value) return value end`);
	}
	...
	console.log(JSON.stringify({ functions, sourceUtf16: source.length, binderMilliseconds, summaryMilliseconds,
