...
export async function runAssistantAccount(kind: StudioRendererKind, canvas: HTMLCanvasElement, capture: (name: string) => Promise<void>, issuer: AccountFixtureControl) {
	...
	await until(() => conversation.state === 'signing-in', 'account: implicit submission requests browser authorization');
	...
	check(conversation.loginCode === undefined && !conversation.canSend, 'browser authorization exposes no code to carry');
	await submitAssistantText(test, '/cancel');
	await until(() => conversation.state === 'ready', 'account: cancel the implicit browser attempt');
	// The rest of this scenario covers the device-code method, for a browser on another machine.
