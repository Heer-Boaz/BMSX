...
import { revealLensRow } from './studio_behavior_picker';
...
export async function testStudioBehaviorSourceGraph(test: StudioFixture): Promise<void> {
	...
	await revealLensRow(test, view, selected.rowKey);
	const top = view.layout.contentTop + (view.selectionIndex - view.scroll) * view.layout.rowHeight;
	const bounds = { left: view.rows[view.selectionIndex].twistieRight, right: view.layout.contentRight,
		top, bottom: top + view.layout.rowHeight };
	await click(bounds);
	await click(view.actionBar.items[0].bounds);
	...
	await click(view.actionBar.items[0].bounds, 6);
	...
	check(view.rows[view.selectionIndex].node === nextSelected && selected.rowKey !== nextSelected.rowKey,
	...
	const retainedRow = view.rows[view.selectionIndex];
	for (let index = 0; index < 30; index += 1) await frame();
	check(view.document === currentDocument && view.rows[view.selectionIndex] === retainedRow,
		'source graph: idle frames neither reparse nor rebuild retained rows');
	await click(view.actionBar.items[0].bounds, 6);
	...
	await click(editorChromeState.tabButtonBounds.get(lens.id)!);
	check(view.selectionIndex === -1, 'source graph: deleting the selected occurrence does not choose its namesake');
	await press('Enter');
	...
	await click(editorChromeState.tabButtonBounds.get(lens.id)!);
	check(view.selectionIndex === -1, 'source graph: ordinary Undo does not invent correspondence for a removed selection');
	await press('ArrowDown');
	check(view.selectionIndex === 0, 'source graph: explicit keyboard navigation starts at the first row after removal');
	await click(editorChromeState.tabButtonBounds.get(code.id)!);
