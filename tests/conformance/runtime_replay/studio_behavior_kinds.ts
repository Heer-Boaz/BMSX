...
export async function testStudioBehaviorKinds(test: StudioFixture): Promise<void> {
	...
	check(model.resource.path === 'player/actioneffects.lua'
		&& lens.view.rows[lens.view.selectionIndex].node.behaviorKind === 'action_effect',
		'behavior kinds: effect selection reveals the actual definition in its resource-owned model');
	const period = lens.view.rows.findIndex(row => row.node.label.startsWith('period_ms ='));
	check(period >= 0 && lens.view.rows.some(row => row.node.label.startsWith('handler =')),
	...
	await runPaletteCommand('Behavior Lens: Open ActionEffect');
	await chooseBehavior(test, 'EFFECT fire_salvo', 'ACTIONEFFECTS');
	check(getActiveTab() === lens && lens.view.rows.some(row => row.node.label === 'period_ms = 80'),
	...
	await runPaletteCommand('Behavior Lens: Open ActionEffect');
	await chooseBehavior(test, 'EFFECT fire_salvo', 'ACTIONEFFECTS');
	check(!lens.view.rows.some(row => row.node.label === 'period_ms = 80'), 'behavior kinds: source Undo refreshes the retained lens');
	...
	await press('Escape');
	check(inputFocus.target === focus && getActiveTab() === lens && lens.view.rows[lens.view.selectionIndex].node === selected,
	...
	check(fsm.kind === 'behavior_lens' && fsm.view.rows[fsm.view.selectionIndex].node.behaviorKind === 'state_machine',
	...
	check(bt.kind === 'behavior_lens' && bt.view.rows[bt.view.selectionIndex].node.behaviorKind === 'behavior_tree',
