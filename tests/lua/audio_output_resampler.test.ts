...
test('host audio resampling starts at the first interpolation window and retains chunk-continuous phase', () => {
	...
	assert.equal(split.pull(splitRing, splitFirst, 17, 48000), 17);
	assert.equal(split.pull(splitRing, splitSecond, 43, 48000), 43);
	assert.equal(batch.pull(batchRing, batched, 60, 48000), 60);
	...
	assert.equal(new AudioOutputResampler().pull(immediateRing, immediateOutput, 1, APU_SAMPLE_RATE_HZ), 1);
...
test('host audio resampling preserves PAL phase across source starvation without publishing silence', () => {
	...
	assert.equal(reference.pull(referenceRing, referenceFirst, outputFramesPerPalFrame - 1, 48000), outputFramesPerPalFrame - 1);
	assert.equal(reference.pull(referenceRing, referenceSecond, outputFramesPerPalFrame, 48000), outputFramesPerPalFrame);
	...
	const producedFirst = starved.pull(starvedRing, starvedFirst, outputFramesPerPalFrame, 48000);
	...
	assert.equal(starved.pull(starvedRing, recoveredSecond, outputFramesPerPalFrame, 48000), outputFramesPerPalFrame);
...
test('host audio resampling discards cached interpolation endpoints after output history is overwritten', () => {
	...
	assert.equal(resampler.pull(ring, first, 2, 48000), 2);
	...
	assert.equal(resampler.pull(ring, afterOverwrite, 1, 48000), 1);
