...
test('written formal bindings have a location per call, distinct from the input value', () => {
	...
	const firstInput = query.contextualize(summary.parameters[0], first);
	assert.equal(summaries.terms.kind(firstInput), TermKind.ContextRoot);
	assert.equal(summaries.terms.base(firstInput), summary.parameters[0]);
	assert.equal(summaries.terms.operand(firstInput), first);
	assert.notEqual(firstInput, argument, 'the call input is a retained flow point, not the first actual');
	assert.equal(query.values.target(query.values.first(firstInput)), argument);
	assert.equal(query.values.next(query.values.first(firstInput)), 0);
	...
	assert.notEqual(firstBinding, firstInput);
	...
	assert.equal(query.values.target(query.values.first(firstBinding)), firstInput, 'entry values initialize the writable binding');
