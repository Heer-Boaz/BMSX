...
function fixture(t: TestContext, source = BT_ORDER_SOURCE, definition = 0) {
	...
	const move = (direction: -1 | 1) => {
		...
		model.pushEditOperations(createLuaTableFieldMoveEdits(model.buffer, model.resource.path, member.table,
			fields.indexOf(member.entries[member.index].field), fields.indexOf(member.entries[member.index + direction].field)));
		refresh();
	};
