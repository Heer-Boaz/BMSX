...
test('source generation and readonly invalidate an in-flight source payload independently of layout refresh', t => {
	...
	assert.equal(beginBehaviorTreeDrag(f.model, f.view, () => assert.fail('unexpected list transfer')), undefined);
	f.model.refreshResource(resource);
	f.model.pushEditOperations([{ offset: f.model.buffer.length, deleteLength: 0, text: '\n@' }]);
	assert.equal(drag.isCurrent(), false);
	assert.equal(beginBehaviorTreeDrag(f.model, f.view, () => assert.fail('unexpected list transfer')), undefined, 'old syntax does not admit a new drag');
	f.refresh(); f.select(0);
	assert.equal(beginBehaviorTreeDrag(f.model, f.view, () => assert.fail('unexpected list transfer')), undefined, 'recovered syntax is not an editable complete constructor');
});
