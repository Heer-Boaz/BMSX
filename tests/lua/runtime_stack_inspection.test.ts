...
return descend(2)`, level);
	...
	const read = t.mock.method(cpu, 'readFrameRegister');
	...
	assert.equal(read.mock.callCount(), 0, 'stack metadata does not read registers');
...
return value, folded`, level);
	...
	const reads = t.mock.method(f.runtime.machine.cpu, 'readFrameRegister');
