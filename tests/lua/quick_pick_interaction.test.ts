...
function fixture(t: TestContext) {
	...
	const open = () => picker.pick('Choose', 'Filter', () => items, item => { accepted = item; });
...
test('picker prepares visible rows lazily across scrolling, query replacement and font/width generations', t => {
	const { picker, key, press, clock } = fixture(t), model = picker.model;
	const first = model.entries[0], hidden = model.entries[50], last = model.entries[99];
	assert.equal(model.entries.filter(row => row.labelText.length !== 0).length, model.endVisibleIndex);
	assert.equal(hidden.labelText, ''); assert.equal(last.labelText, '');
	key('ControlLeft', true); press('End'); key('ControlLeft', false); picker.update();
	assert.equal(last.labelText, last.item.label);
	assert.equal(hidden.labelText, '', 'scrolling does not prepare the invisible catalog between endpoints');
	...
	assert.equal(model.list.rows[0], hidden);
