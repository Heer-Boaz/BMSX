...
return original.field, actual.field`;
		...
		const frame = query.instantiate(call.site, summary.id, 0, 0, call.arguments, call.result);
		if (!demandBeforeCall) query.demandName(field);
		const write = query.writes.firstName(field);
		assert.notEqual(write, 0);
		const receiver = query.contextualize(summary.parameters[0], frame);
		assert.equal(query.writes.base(write), receiver);
		const input = query.values.first(receiver);
		assert.equal(query.values.target(input), actualTerm);
		assert.equal(query.values.next(input), 0, 'only the actual receiver flows into this frame');
