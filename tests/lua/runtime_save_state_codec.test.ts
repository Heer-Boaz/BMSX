		...
		machineState: {
			machine: {
				...
				gxGpu: {
					...
					gp0PolylineOpcode: 0x58,
					gp0PolylineColorWord: 0x00010203,
					gp0PolylinePositionWord: 0x00050006,
					gp0PolylineNextColorWord: 0x00040506,
				...
				systemControl: {
					...
					supervisorFaultBadAddressWord: 0,
					supervisorFaultLuaReasonWord: 0,
					supervisorFaultDomainWord: 0,
				},
			},
			frameScheduler: {
...
test('runtime save-state codec stores READY GPUREAD bytes and rejects backend-only phases', () => {
	...
	assert.throws(
		() => decodeRuntimeSaveState(
			encodeRuntimeSaveState(submitted),
			PSX_MACHINE_SPEC.ramBytes,
			PSX_MACHINE_SPEC.gxGpuVramBytes,
		),
