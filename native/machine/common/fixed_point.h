...
inline void decodeSignedQ16_16WordsInto(f32* target, size_t targetBase, const u32* words, size_t wordBase, size_t count) {
	for (size_t index = 0u; index < count; ++index) {
		target[targetBase + index] = decodeSignedQ16_16(words[wordBase + index]);
	}
}
