...
void bmsx_audio_queue_push(BmsxAudioQueue* queue, const int16_t* data, size_t frames) {
	...
	const size_t overflow = queue->used_frames + frames > queue->capacity_frames
		? queue->used_frames + frames - queue->capacity_frames
		: 0u;
	queue->read_frame = (queue->read_frame + overflow) % queue->capacity_frames;
	queue->used_frames -= overflow;
	...
	queue->write_frame = (queue->write_frame + frames) % queue->capacity_frames;
	queue->used_frames += frames;
