...
import { collectSourceFiles, resolveInputPath } from '../lib/file_scan';
...
function parseClangTidyOutput(output: string, issues: LintIssue[]): void {
	// disable-next-line newline_normalization_pattern -- clang-tidy stdout is a line-oriented tool boundary.
...
function parseCheckOutput(output: string, issues: LintIssue[]): void {
	// disable-next-line newline_normalization_pattern -- cppcheck stdout is a line-oriented tool boundary.
	const lines = output.split('\n');
	for (let i = 0; i < lines.length; i += 1) {
		const line = lines[i];
		if (line.length === 0) {
