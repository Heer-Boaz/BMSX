...
```bash
npm run ast-transform:convert -- machine/ts/ide/cart_editor.ts
# or
node scripts/ast-transform/convert_class_members.js machine/ts/ide/cart_editor.ts

# ts-morph converter (recommended for cross-file rewiring)
npm run ast-transform:morph -- machine/ts/ide/cart_editor.ts
# or, with project awareness and cross-file rewiring (updates imports/usages)
npm run ast-transform:morph:project -- machine/ts/ide/cart_editor.ts
# optionally, conservatively flatten simple instance usages in project mode
npm run ast-transform:morph:project -- --flatten-instances machine/ts/ide/cart_editor.ts
```
