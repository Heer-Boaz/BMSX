...
```bash
npm run ast-transform:convert -- src/bmsx/ide/console_cart_editor.ts
# or
node scripts/ast-transform/convert_class_members.js src/bmsx/ide/console_cart_editor.ts

# ts-morph converter (recommended for cross-file rewiring)
npm run ast-transform:morph -- src/bmsx/ide/console_cart_editor.ts
# or, with project awareness and cross-file rewiring (updates imports/usages)
npm run ast-transform:morph:project -- src/bmsx/ide/console_cart_editor.ts
# optionally, conservatively flatten simple instance usages in project mode
npm run ast-transform:morph:project -- --flatten-instances src/bmsx/ide/console_cart_editor.ts
```
