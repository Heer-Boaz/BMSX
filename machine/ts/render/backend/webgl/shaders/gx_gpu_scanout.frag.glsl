...
float display2RawWordAtLogical(float x, float y) {
	vec2 vramCoord = mod(vec2(x, y), VRAM_SIZE);
	vec2 texcoord = vec2((vramCoord.x + 0.5) / VRAM_SIZE.x, 1.0 - (vramCoord.y + 0.5) / VRAM_SIZE.y);
	return rawWordFromPixel(texture(u_display2_vram, texcoord));
}
...
void main() {
	...
	vec4 primary = texture(u_vram, vec2(gl_FragCoord.x / u_interlace.z, (storedY + 0.5) / u_interlace.y));
	outputColor = composeDisplay2(primary, floor(gl_FragCoord.x), outputY);
	...
	if (u_display2.w > 0.5) {
		outputColor = composeDisplay2(vec4(0.0, 0.0, 0.0, 1.0), sourceX, outputY);
		return;
	}
	...
	outputColor = composeDisplay2(vec4(rgb8 / 255.0, 1.0), sourceX, outputY);
#endif
}
