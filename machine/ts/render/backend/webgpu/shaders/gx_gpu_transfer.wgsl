...
struct VSIn { @location(0) position: vec2<f32>, @location(1) sourceOffset: vec2<f32> };
struct VSOut { @builtin(position) position: vec4<f32>, @location(0) @interpolate(flat) sourceOffset: vec2<f32> };
...
fn vs_main(input: VSIn) -> VSOut {
	...
	out.sourceOffset = input.sourceOffset;
	return out;
}
