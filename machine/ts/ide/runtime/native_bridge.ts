...
import { convertToError, isLuaCallSignal, LuaValue, LuaTable, isLuaTable, createLuaTable, LuaNativeValue, isLuaFunctionValue, isPlainObject, resolveNativeTypeName, isLuaNativeMemberHandle, LuaFunctionValue } from '../../lua/value';
import type { LuaInterpreter } from '../../lua/runtime';
import { Table, asStringId, valueIsString, type Closure, type NativeFunction, type NativeObject, type Value, isNativeFunction, isNativeObject } from '../../machine/cpu/cpu';
import { LuaFunctionRedirectCache } from './handler_registry';
import type { Runtime } from '../../machine/runtime/runtime';
import type { LuaInteropAdapter, LuaMarshalContext } from '../../lua/interop';
...
export class LuaJsBridge implements LuaInteropAdapter {
	...
	public deserializeLuaSnapshotKey(raw: unknown, resolver?: (value: unknown) => LuaValue): LuaValue {
		...
		if (raw && typeof raw === 'object' && (raw as { __native_member_handle__?: boolean }).__native_member_handle__) {
			...
			return this.bridge.luaInterpreter.createNativeMemberHandle(handleTarget, path);
		}
		if (resolver) {
			return resolver(raw);
		}
	...
	public wrapFunctionsInValue(
		...
		if (isLuaFunctionValue(value)) {
			...
			return this.bridge.luaFunctionRedirectCache.getOrCreate(moduleId, path, value);
		}
		if (!isLuaTable(value)) {
			return value;
		}
...
export function buildMarshalContext(): LuaMarshalContext {
	return { moduleId: machineManager.sourceState.currentPath, path: [] };
}
...
function nativeKeysEqual(bridge: RuntimeNativeBridge, left: Value, right: Value): boolean {
	...
	if (valueIsString(left) && valueIsString(right)) {
		return bridge.runtime.machine.cpu.stringPool.toString(asStringId(left)) === bridge.runtime.machine.cpu.stringPool.toString(asStringId(right));
	}
	if (typeof left === 'number' && valueIsString(right)) {
		return String(left) === bridge.runtime.machine.cpu.stringPool.toString(asStringId(right));
	}
	if (valueIsString(left) && typeof right === 'number') {
		return bridge.runtime.machine.cpu.stringPool.toString(asStringId(left)) === String(right);
	}
	return false;
}
...
export function toNativeValue(bridge: RuntimeNativeBridge, value: Value, context: LuaMarshalContext, visited: TableMarshalVisited): unknown {
	...
	if (valueIsString(value)) {
		return bridge.runtime.machine.cpu.stringPool.toString(asStringId(value));
	}
	if (value instanceof Table) {
		return tableToNative(bridge, value, context, visited);
	}
