...
void HostFaultState::publishStartup(const std::string& error) {
	m_runtime.setGlobal("sys_host_fault_message", valueString(m_runtime.machine.cpu.stringPool().intern(error)));
...
void HostFaultState::clear() {
	m_runtime.setGlobal("sys_host_fault_message", valueNil());
