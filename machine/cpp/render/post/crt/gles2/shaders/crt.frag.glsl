...
BlurContrast applyBlurAndContrast(vec2 uv, vec2 sourcePixel){
	...
	left = texture2D(u_texture, uv - u_srcTexel).rgb;
	center = texture2D(u_texture, uv + vec2(0.0, -u_srcTexel.y)).rgb;
	right = texture2D(u_texture, uv + vec2(u_srcTexel.x, -u_srcTexel.y)).rgb;
	...
	left = texture2D(u_texture, uv + vec2(-u_srcTexel.x, 0.0)).rgb;
	center = texture2D(u_texture, uv).rgb;
	vec3 centerColor = center;
	right = texture2D(u_texture, uv + vec2(u_srcTexel.x, 0.0)).rgb;
	...
	left = texture2D(u_texture, uv + vec2(-u_srcTexel.x, u_srcTexel.y)).rgb;
	center = texture2D(u_texture, uv + vec2(0.0, u_srcTexel.y)).rgb;
	right = texture2D(u_texture, uv + u_srcTexel).rgb;
...
void main(){
	...
	if (u_enableBlur || u_enableFringing || u_enableAperture || u_enableScanlines) {
		...
		bc.center = texture2D(u_texture, v_texcoord).rgb;
